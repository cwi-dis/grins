...
##                     raise 'Parent window does not contain child', (self, pchan)
    ...
    def play_0(self, node, curtime):
        ...
        if debug: print curtime, 'Channel.play_0('+`self`+','+`node`+')'
...
##             print 'stopping playing node first',`self._played_node`
    ...
    def onclick(self, node):
        ...
        for arc in node.sched_children:
            if arc.event == ACTIVATEEVENT and arc.refnode() is node:
                self._playcontext.sched_arcs(node, self._scheduler.timefunc(), ACTIVATEEVENT, external = 1)

    def event(self, event, curtime = None):
        ...
        if curtime is None:
            curtime = self._scheduler.timefunc()
        sctx.sched_arcs(node, curtime, event, external = 1)
    ...
    def play_1(self, curtime):
        ...
        if debug: print curtime, 'Channel.play_1('+`self`+')'
...
##                     self.playdone, (0, start_time+self.armed_duration))
    ...
    def playdone(self, outside_induced, curtime):
        ...
        if debug: print curtime, 'Channel.playdone('+`self`+')'
        if self._playstate != PLAYING:
    ...
    def freeze(self, node, curtime):
        ...
        if debug: print curtime, 'Channel.freeze'+`self,node`
    ...
    def playstop(self, curtime):
        ...
        if debug: print curtime, 'Channel.playstop('+`self`+')'
        if self._playstate != PLAYING:
    ...
    def play_anchor(self, anchor, curtime):
        if debug: print curtime, 'Channel.play_anchor',self,anchor,curtime
    ...
    def stop_anchor(self, anchor, curtime):
        if debug: print curtime, 'Channel.stop_anchor',self,anchor,curtime
        actuate = MMAttrdefs.getattr(anchor, 'actuate')
        if actuate == 'onLoad':
            return
    ...
    def play(self, node, curtime):
        ...
        # anymore.
        if debug: print curtime, 'Channel.play('+`self`+','+`node`+')'
        if node.GetType() == 'anchor':
    ...
    def stopplay(self, node, curtime):
        ...
        # Node is only passed to do consistency checking.
        if debug: print curtime, 'Channel.stopplay('+`self`+','+`node`+')'
...
##             print 'node was not the playing node '+`self,node,self._played_node`
    ...
    def stopcontext(self, ctx, curtime):
        ...
        # complete idle state.
        if debug: print curtime, 'Channel.stopcontext'+`(self, ctx)`
    ...
    def getstring(self, node):
        if node.type == 'imm':
            ...
            if node.context.state is not None:
                data = node.context.state.interpolate(data)
            return data
        elif node.type == 'ext':
...
##         return MMAttrdefs.getattr(node, 'fgcolor', animated)
        ...
        if fgcolor is not None:
            return fgcolor
        paramgroup = node.GetAttrDef('paramgroup', [])
        for name in paramgroup:
            if node.context.paramgroups.has_key(name) and \
               node.context.paramgroups[name].has_key('color'):
                val = node.context.paramgroups[name]['color']
                if node.context.state is not None:
                    val = node.context.state.interpolate(val)
                import colors
                return colors.convert_color(val)
        r,g,b = self.getbgcolor(node, animated)
        if MMAttrdefs.getattr(node, 'transparent', animated):
            return settings.get('fgcolor')
        elif r*r + g*g + b*b < 3*128*128:
            return 255,255,255
        else:
            return 0,0,0

    def gethicolor(self, node, animated=0):
        # The hicolor property doesn't exist anymore, but the HTML
        # channels still use it to set the hyperlink color. Therefore
        # this method still exists, and returns red.
        return (255, 0, 0)
...
##                          (self.unhighlight, ())))
            ...
            if self.want_default_colormap:
                self.window = pchan.window.newcmwindow(
                    pgeom,
                ...
                self.window = pchan.window.newwindow(
                    pgeom,
                    transparent = self.__transparent,
                    z = self.__z,
                    units = units,
            ...
            bgimg = self._attrdict.get('backgroundImage', 'none')
            if bgimg != 'none':
                bgrep = self._attrdict.get('backgroundRepeat', 'repeat')
                self.window.bgimage(bgimg, bgrep)
        else:
...
#                       self.window.updatecoordinates(wingeom, units)
    ...
    def play(self, node, curtime):
        if debug: print curtime, 'ChannelWindow.play('+`self`+','+`node`+')'
        if node.GetType() == 'anchor':
            self.play_anchor(node, curtime)
            return
        self.play_0(node, curtime)
    ...
    def stopplay(self, node, curtime):
        if debug: print curtime, 'ChannelWindow.stopplay('+`self`+','+`node`+')'
        if node.GetType() == 'anchor':
...
class ChannelAsync(Channel):
    ...
    def play(self, node, curtime):
        if debug: print curtime, 'ChannelAsync.play('+`self`+','+`node`+')'
        if node.GetType() == 'anchor':
            self.play_anchor(node, curtime)
            return
        self.play_0(node, curtime)
...
class ChannelWindowAsync(ChannelWindow):
    def play(self, node, curtime):
        if debug: print curtime, 'ChannelWindowAsync.play('+`self`+','+`node`+')'
        if node.GetType() == 'anchor':
            self.play_anchor(node, curtime)
            return
        self.play_0(node, curtime)
