...
##             self.parent.channels_in_use.remove(ch)
    ...
    def start(self, s_node, s_args, timestamp = None):
        ...
        if playroot.context.state is not None:
            playroot.context.state.start()
    ...
    def sched_arc(self, node, arc, curtime,
        ...
        if arc.wallclock is not None:
            ...
            # Retrieve the timestamp since it was not supplied.
            timestamp = self.parent.timefunc()
        if arc.ismin:
            list = []
        elif arc.isstart:
            ...
            if a.timestamp > timestamp + arc.delay:
                ...
                if a.isstart:
                    if a.dstnode.GetSchedParent():
                        srdict = a.dstnode.GetSchedParent().gensr_child(curtime, a.dstnode, runchild = False, sctx = self)
        ...
        if arc.qid is None and (not arc.isstart or arc.dstnode.playing != MMStates.PLAYING or timestamp + arc.delay != arc.dstnode.start_time):
            if arc.isstart:
                ...
                if pnode is not None:
                    srdict = pnode.gensr_child(curtime, arc.dstnode, runchild = False, sctx = self)
...
##                 prio = False
            ...
            if arc.isstart:
                if arc.dstnode.start_time is None or arc.dstnode.start_time > arc.timestamp:
                    arc.dstnode.set_start_time(arc.timestamp, False)
    ...
    def sched_arcs(self, node, curtime, event = None, marker = None, timestamp = None, depth = 0, external = False):
        ...
        for arc in node.sched_children:
            # this tests whether the dependant arc can be
            ...
            do_continue = False # on old Python we can't continue from inside try/except
            try:
    ...
    def trigger(self, curtime, arc, node = None, path = None, timestamp = None):
        ...
        parent = self.parent
        parent.setpaused(True, curtime, False)
        if __debug__:
        ...
        if arc is not None:
            ...
            if not arc.isstart and node.playing in (MMStates.IDLE, MMStates.PLAYED):
                ...
                self.cancelarc(arc, timestamp, propagate = True, syncbase = False)
                parent.updatetimer(curtime)
                return
            if node.has_min and not arc.isstart and not arc.ismin:
                # must delay this arc
                node.delayed_end = True
##                 self.cancelarc(arc, timestamp, propagate = True)
                if arc.isdur:
                    # fill between end of simple duration and end of active duration
                    self.do_terminate(node, curtime, timestamp, fill = node.GetFill(), skip_cancel = True)
...
##                     arc.srcnode.sched_children.remove(arc)
        ...
        if arc is not None:
            if arc.ismin:
                ...
                node.delayed_play_done = node.delayed_end = False
...
##                         self.sched_arc(node, arc, curtime, event='begin', timestamp=timestamp)
        ...
        if not found:
            ...
            if endtime is not None and endtime >= 0 and ((node.type == 'state' and endtime < curtime) or (node.type != 'state' and endtime <= curtime)):
                found = False
            elif endlist:
                found = False
                for a in endlist:
...
##         node.cleanup_sched(parent)
        ...
        if hasattr(node, 'endtime'):
            ...
            if tm == timestamp or ev in ('begin', 'end'):
                runchild = False
            del node.endtime
        ndur = node.calcfullduration(self, ignoremin = True)
        ...
        if maxtime >= 0:
            if ndur >= 0:
                ...
                ndur = maxtime
        if ndur >= 0 and timestamp + ndur <= parent.timefunc() and node.type != 'state' and MMAttrdefs.getattr(node, 'erase') != 'never' and node.GetFill() == 'remove':
            runchild = False
...
##             self.sched_arcs(node, curtime, 'end', timestamp=timestamp)
    ...
    def cancelarc(self, arc, timestamp, cancel_gensr = True, propagate = False, syncbase = True):
        if __debug__:
...
##                     c.resetall(parent)
        ...
        if mint is not None:
            t, arc = mint
            self.sched_arc(node, arc, curtime, 'begin', timestamp = t - arc.delay, force = True)
        node.__terminating = False
        if __debug__:
    ...
    def flushqueue(self, curtime, xxx = False):
        parent = self.parent
        if __debug__:
...
##             self.toplevel.setwaiting()
    ...
    def updatetimer(self, curtime):
        ...
        if not self.playing:
            ...
            self.setpaused(False, curtime, False)
            return
        if __debug__:
            if debugtimer: print 'updatetimer: delay=', delay
        if delay == 0:
            self.setpaused(True, curtime, False)
        else:
            self.setpaused(False, curtime, False)
        ...
        if action == SR.LOOPSTART:
            ...
            if pnode is not None and pnode.GetType() == 'excl' and pnode.pausestack and arg not in pnode.pausestack:
                ...
                for ch in arg.GetSchedChildren():
                    sctx.do_terminate(ch, curtime, timestamp, chkevent = False)
                    if not self.playing:
                        return
            else:
                sctx.do_terminate(arg, curtime, timestamp, fill = arg.GetFill(), chkevent = False)
                if not self.playing:
                    return
...
##                     sctx.sched_arcs(arg, curtime, 'end', timestamp=timestamp+adur)
    ...
    def do_play(self, sctx, node, curtime, timestamp):
        ...
        if n is not None:
            sctx.do_terminate(n, curtime, timestamp, ignore_erase = True)
...
##         print 'LOOPRESTART', node
    ...
    def setpaused(self, paused, curtime = None, propagate = True):
        if curtime is None:
