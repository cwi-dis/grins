...
##		self.parent.updatetimer()
	...
	def event(self, ev, timestamp):
		if debugevents: print 'event', SR.ev2string(ev), timestamp
	...
	def trigger(self, arc):
		...
		if not arc.isstart:
			...
			if pnode.type == 'excl' and pnode.pausestack:
				...
				self.parent.do_resume(self, node, timestamp)
			return
		if not node.parent:
		...
		if pnode.type == 'excl':
			...
			for sib in pnode.GetSchedChildren():
				...
				if sib.playing == MMStates.PLAYING:
					...
					if debugevents: print `sib`,`node`,pcmp,p1,p2
					if pcmp == 0:
						if p1[0].type == 'excl':
							# no priorityClass
							action = 'stop'
					...
					if debugevents: print 'action',action
					if action == 'stop':
		...
		if runchild:
			...
			if debugevents: print 'trigger, no run'
##			self.parent.sched_arcs(self, node, 'begin', timestamp=timestamp)
##			self.parent.sched_arcs(self, node, 'end', timestamp=timestamp)
			self.parent.event(self, (SR.SCHED_DONE, node), timestamp)

	def queuesrlist(self, srlist, timestamp = None):
		for sr in srlist:
			if debugevents: print '  queue', SR.ev2string(sr), timestamp
			if sr[0] == SR.PLAY:
				prio = PRIO_START
...
##		raise error, 'arm_ready: unused channel %s' % chan
	...
	def sched_arcs(self, sctx, node, event = None, marker = None, timestamp = None):
		if debugevents: print 'sched_arcs',`node`,event,marker,timestamp
		if timestamp is None:
...
##				arc.timestamp = None
			...
			for a in list:
				...
				if a.timestamp > timestamp + arc.delay:
					if debugevents: print 'sched_arcs: cancel',`a`
					self.cancel(a.qid)
					a.qid = None
					if a.isstart:
						if a.dstnode.parent:
...
##						arc.dstnode.parent.scheduled_children = arc.dstnode.parent.scheduled_children + 1
					...
					if debugevents: print 'scheduled_children+1',`arc.dstnode`,`node`,event
					arc.dstnode.scheduled_children = arc.dstnode.scheduled_children + 1
				arc.timestamp = timestamp+arc.delay
				arc.qid = self.enterabs(arc.timestamp, 0, sctx.trigger, (arc,))
	...
	def runone(self, (sctx, todo, dummy), timestamp = None):
		...
		if debugevents: print 'exec: ', SR.ev2string(todo), timestamp
		if timestamp is None:
			timestamp = self.timefunc()
		action, arg = todo
		if action == SR.PLAY:
...
## 			self.remove_terminate(sctx, arg)
			...
			self.do_terminate(sctx, arg, timestamp)
			...
			if action == SR.SCHED_STOPPING:
				if arg.scheduled_children:
					if debugevents: print 'not stopping',`arg`,arg.scheduled_children
					return
...
##					self.remove_terminate(sctx, arg)
				...
				for ch in arg.GetSchedChildren():
					ch.reset()
				if arg.fullduration is None or arg.fullduration < 0:
				...
				if arg.fullduration is not None and arg.fullduration >= 0:
...
##		for ev in [(SR.TERMINATE, node), (SR.TERMINATE, node.looping_body_self)]:
	...
	def do_play(self, sctx, node, timestamp):
		...
		node.startplay(sctx)
##		self.sched_arcs(sctx, node, 'begin', timestamp = timestamp)
		if node.fullduration is not None and node.fullduration >= 0:
			self.sched_arcs(sctx, node, 'end', timestamp = timestamp+node.fullduration)
...
##			self.event(sctx, (SR.TERMINATE, node), timestamp)
##		else:
##			# terminate node
...
##				sctx.arm_ready(chan)
	...
	def do_pause(self, sctx, pnode, node, action, timestamp):
	...
	def pause_play(self, node, action, timestamp):
		...
		if node.type in leaftypes and getchannelfunc:
			...
			if chan:
				if debugevents: print 'freeze',`node`
	...
	def do_resume(self, sctx, node, timestamp):
		...
		if sctx.srdict.has_key(ev):
			...
			self.resume_play(sctx, node, timestamp)
			...
			for c in node.GetSchedChildren():
				if c.playing == MMStates.PAUSED and \
				   c not in node.pausestack:
					self.resume_play(sctx, c, timestamp)
		else:
			for c in node.GetSchedChildren():
				self.resume_play(sctx, c, timestamp)
