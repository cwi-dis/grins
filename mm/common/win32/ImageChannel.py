...
class ImageChannel(ChannelWindow):
	...
	def do_arm(self, node, same=0):
		...
		try:
			self._arm_imbox = self.armed_display.display_image_from_file(f, scale = MMAttrdefs.getattr(node, 'scale'), crop = MMAttrdefs.getattr(node, 'crop'), center = MMAttrdefs.getattr(node, 'center'), tras = result[1])
