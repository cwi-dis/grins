...
# And an attribute editor for paramgroups
def showparamgroupattreditor(toplevel, pgname, initattr = None):
    try:
        attreditor = toplevel.context.paramgroups[pgname]['__attreditor']
    except KeyError:
        attreditor = AttrEditor(ParamgroupWrapper(toplevel, pgname), initattr = initattr)
        toplevel.context.paramgroups[pgname]['__attreditor'] = attreditor
    else:
        attreditor.pop()

def hasparamgroupattreditor(toplevel, pgname):
    try:
        attreditor = toplevel.context.paramgroups[pgname]['__attreditor']
    except KeyError:
        return 0
...
class DocumentWrapper(Wrapper):
    ...
    __publishnames = ['project_ftp_host', 'project_ftp_user',
                      'project_ftp_dir', 'project_ftp_host_media',
                      'project_ftp_user_media', 'project_ftp_dir_media',
                      'project_smil_url', 'project_web_url']
    __qtnames = ['autoplay', 'qttimeslider', 'qtnext', 'qtchaptermode',
                 'immediateinstantiation']
    __templatenames = ['template_name', 'template_description',
                       'template_snapshot']
...
class TermnodenameAttrEditorField(PopupAttrEditorFieldWithUndefined):
    ...
    def getoptions(self):
        ...
        list.sort()
        extras = ['LAST', 'FIRST']
        if self.wrapper.context.smilversion >= 20:
            extras.append('ALL')
