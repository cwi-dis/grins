...
class HelpWindow(BasicDialog):
	...
	# Next time, start with a clean sheet...
	self.topics = None
	self.return_stack = []
	...
	if topic <> self.curtopic and self.curtopic <> '' \
		and type(subtopic) <> type(1):
	    num = self.help.get_browser()
	    for x in self.return_stack[:]:
		if x[0] == self.curtopic: self.return_stack.remove(x)
	    self.return_stack.append((self.curtopic,num))
