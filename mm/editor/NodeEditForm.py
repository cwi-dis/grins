...
def mk_form_convertform (mod):
	...
	#
	input_filename = convertform.add_input(ALWAYS_INPUT,180.0,50.0,270.0,30.0,'Filename:')
	input_filename.set_call_back(mod.callback_input, 0)
	...
	#
	obj = convertform.add_button(NORMAL_BUTTON,110.0,10.0,90.0,30.0,'Browser...')
	obj.set_call_back(mod.callback_browser, 0)
	#
	return convertform
...
def mk_form_err (mod,err):
	...
	#
	obj = err.add_text(NORMAL_TEXT,200.0,10.0,140.0,30.0,'File already exists!')
	#
	button_overwrite = err.add_button(NORMAL_BUTTON,330.0,10.0,120.0,30.0,'Overwrite')
	button_overwrite.set_call_back(mod.callback_convert, 0)
