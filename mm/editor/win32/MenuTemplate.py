...
MENUBAR=(
	('&File', (
		...
		(LIGHT, SEP,),
		(SMIL_QT, ENTRY, 'Publish for &QuickTime...', None, EXPORT_QT),
		(SMIL_QT, ENTRY, 'Pu&blish for QuickTime and upload...', None, UPLOAD_QT),
		(SMIL_G2, ENTRY, 'Publish for &G2...', None, EXPORT_G2),
		(SMIL_G2, ENTRY, 'Pu&blish for G2 and upload...', None, UPLOAD_G2),
		(LIGHT, SEP,),
...
##		(SMIL, ENTRY, '&Info...', 'I', INFO),
	...
	('&Insert', (
		...
		(CMP_G2, CASCADE, 'Sli&deshow node', (
			(CMP_G2, ENTRY, '&Before', None, NEW_BEFORE_SLIDESHOW),
			(CMP_G2, ENTRY, '&After', None, NEW_AFTER_SLIDESHOW),
			(CMP_G2, ENTRY, '&Within', None, NEW_UNDER_SLIDESHOW),
		)),
...
POPUP_HVIEW_SLIDE = (
		# XXXX Need to add the "new xxx node" commands for the
		# light version
		(CMP_G2, ENTRY, 'Cu&t', None, CUT),
		(CMP_G2, ENTRY, '&Copy', None, COPY),
		(CMP_G2, ENTRY, '&Paste', None, PASTE_AFTER),
		(CMP_G2, ENTRY, 'Paste bef&ore', None, PASTE_BEFORE),
		(CMP_G2, ENTRY, 'Pa&ste file', None, PASTE_FILE),
		(CMP_G2, SEP,),
		(CMP_G2, ENTRY, '&Delete', None, DELETE),
		(CMP_G2, CASCADE, 'Insert Image &Node', (
			(CMP_G2, ENTRY, '&Before', None, NEW_BEFORE_IMAGE),
			(CMP_G2, ENTRY, '&After', None, NEW_AFTER_IMAGE),
			)),
		(CMP_G2, SEP,),
		(CMP_G2, ENTRY, 'P&roperties...', None, ATTRIBUTES),
		(CMP_G2, ENTRY, '&Edit content', None, CONTENT),
)
...
POPUP_CVIEW_SYNCARC = (
		(SMIL, ENTRY, '&Properties...', None, ATTRIBUTES),
		(SMIL, SEP,),
		(LIGHT, ENTRY, '&Delete', None, DELETE),
)
