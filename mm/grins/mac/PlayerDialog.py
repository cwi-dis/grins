...
##			self.__subwins[1].create_menu(menu, title = _titles[1])
	...
	def setstate(self, state):
		...
		self.__state = state
		if self.__window is not None:
			self.__window.setbutton(0, self.__state == PLAYING)
			self.__window.setbutton(1, self.__state == STOPPED)
			self.__window.setbutton(2, self.__state == PAUSING)
	...
	def show(self):
		...
		cmdlist = [
			...
			(UserCmd.STOP, (self.stop_callback, ())),
			(UserCmd.PAUSE, (self.pause_callback, ())),
