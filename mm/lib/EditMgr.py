...
##         self.next_focus = []    # This object cannot have focus (yet)
    ...
    #
    # Paramgroups operations
    #
    def addparamgroup(self, name, value):
        self.attrs_changed = 1
        if self.context.paramgroups.has_key(name):
            raise MMExc.AssertError, \
                  'duplicate paramgroup name in addparamgroup'
        self.addstep('addparamgroup', name)
        self.context.addparamgroup(name, value)
##         self.next_focus = []    # This object cannot have focus (yet)

    def undo_addparamgroup(self, name):
        self.delparamgroup(name)

    def clean_addparamgroup(self, name):
        pass

    def delparamgroup(self, name):
        self.attrs_changed = 1
        paramgroup = self.context.paramgroups.get(name)
        if paramgroup is None:
            raise MMExc.AssertError, 'unknown paramgroup in delparamgroup'
        self.addstep('delparamgroup', name, paramgroup)
        self.context.delparamgroup(name)
##         self.next_focus = []    # This object cannot have focus (yet)

    def undo_delparamgroup(self, name, paramgroup):
        self.attrs_changed = 1
        self.addparamgroup(name, paramgroup)
        for key, val in paramgroup.items():
            self.setparamgroupvalue(name, key, val)

    def clean_delparamgroup(self, name, paramgroup):
        pass

    def setparamgroupname(self, name, newname):
        self.attrs_changed = 1
        if newname == name:
            return          # no change
        if not self.context.paramgroups.has_key(name):
            raise MMExc.AssertError, \
                  'unknown paramgroup name in setparamgroupname'
        if self.context.paramgroups.has_key(newname):
            raise MMExc.AssertError, \
                  'name already in use in setparamgroupname'
        self.addstep('setparamgroupname', name, newname)
        self.context.setparamgroupname(name, newname)
##         self.next_focus = []    # This object cannot have focus (yet)

    def undo_setparamgroupname(self, oldname, name):
        self.setparamgroupname(name, oldname)

    def clean_setparamgroupname(self, oldname, name):
        pass

    def setparamgroupvalue(self, name, key, value):
        self.attrs_changed = 1
        if not self.context.paramgroups.has_key(name):
            raise MMExc.AssertError, \
                  'unknown paramgroup name in setparamgroupname'
        dict = self.context.paramgroups[name]
        oldvalue = dict.get(key)
        if oldvalue == value:
            return
        dict[key] = value
        if value is None:
            del dict[key]
        self.addstep('setparamgroupvalue', name, key, oldvalue)
##         self.next_focus = []    # This object cannot have focus (yet)

    def undo_setparamgroupvalue(self, name, key, oldvalue):
        self.setparamgroupvalue(name, key, oldvalue)

    def clean_setparamgroupvalue(self, name, key, oldvalue):
        pass
