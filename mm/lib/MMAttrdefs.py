...
def getattr(node, attrname, animated = 0):
    ...
    if attrstats is not None:
        attrstats[attrname] = attrstats.get(attrname, 0) + 1
    # Check the cache
    if not animated:
        if hasattr(node, 'attrcache'):
            if node.attrcache.has_key(attrname):
                attrvalue = node.attrcache[attrname]
                if node.context.state is not None and type(attrvalue) is type(''):
                    return node.context.state.interpolate(attrvalue)
                else:
                    return attrvalue
        else:
            node.attrcache = {}
    ...
    if node.context.state is not None and type(attrvalue) is type(''):
        attrvalue = node.context.state.interpolate(attrvalue)
