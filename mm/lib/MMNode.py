...
class MMNodeContext:
    ...
    def __init__(self, nodeclass):
        self.smilversion = 0            # major*10 + minor, 0 = unknown
    ...
    def findurl(self, url):
        ...
        if self.state is not None:
            url = self.state.interpolate(url)
        if self.baseurl:
    ...
    def computeMimeType(self, nodeType, url = None, specifiedMimeType = None):
        if nodeType != 'imm' and url is not None:
            import urlparse
            scheme = urlparse.urlparse(url)[0]
            if scheme in ('http', 'https'):
                specifiedMimeType = None # ignore document-specified type
        if specifiedMimeType is not None:
...
##                 c[key] = val
    ...
    #
    # paramGroup administration
    #
    def addparamgroups(self, list):
        for name, valuedict in list:
            self.paramgroups[name] = valuedict

    def addparamgroup(self, name, valuedict):
        if self.paramgroups.has_key(name):
            raise CheckError, 'addparamgroup: existing name'
        self.paramgroups[name] = valuedict

    def delparamgroup(self, name):
        if not self.paramgroups.has_key(name):
            raise CheckError, 'delparamgroup: non-existing name'
        del self.paramgroups[name]

    def setparamgroupname(self, oldname, newname):
        if newname == oldname:
            return          # No change
        if self.paramgroups.has_key(newname):
            raise CheckError, 'setparamgroup: existing name'
        if not self.paramgroups.has_key(oldname):
            raise CheckError, 'setparamgroup: non-existing name'
        self.paramgroups[newname] = self.paramgroups[oldname]
        del self.paramgroups[oldname]
        # Patch references to this paramgroup in nodes
        for uid in self.uidmap.keys():
            n = self.uidmap[uid]
            pmlist = n.GetRawAttrDef('paramgroup', None)
            if pmlist and oldname in pmlist:
                newpmlist = []
                for pm in pmlist:
                    if pm == oldname:
                        newpmlist.append(newname)
                    else:
                        newpmlist.append(pm)
                n.SetAttr('paramgroup', newpmlist)

    #
    # submission administration
    #
    def addsubmissions(self, list):
        for name, valuedict in list:
            self.submissions[name] = valuedict

    def addsubmission(self, name, valuedict):
        if self.submissions.has_key(name):
            raise CheckError, 'addsubmission: existing name'
        self.submissions[name] = valuedict

    def delsubmission(self, name):
        if not self.submissions.has_key(name):
            raise CheckError, 'delsubmission: non-existing name'
        del self.submissions[name]

    def setsubmissionname(self, oldname, newname):
        if newname == oldname:
            return                      # No change
        if self.submissions.has_key(newname):
            raise CheckError, 'setsubmission: existing name'
        if not self.submissions.has_key(oldname):
            raise CheckError, 'setsubmission: non-existing name'
        self.submissions[newname] = self.submissions[oldname]
        del self.submissions[oldname]
        # Patch references to this submission in nodes
        for uid in self.uidmap.keys():
            n = self.uidmap[uid]
            slist = n.GetRawAttrDef('state_submission', None)
            if slist and oldname in slist:
                newslist = []
                for s in slist:
                    if s == oldname:
                        newslist.append(newname)
                    else:
                        newslist.append(s)
                n.SetAttr('state_submission', newslist)
...
class MMSyncArc:
    def __init__(self, dstnode, action, srcnode = None,
                 ...
                 accesskey = None, statechange = None, contentchange = None,
                 delay = None, implicit = 0):
        ...
        if __debug__:
            ...
            if wallclock is not None:
                ...
                assert statechange is None
                assert contentchange is None
                assert delay == 0
    ...
    def __repr__(self):
        ...
        if self.delay is None:
            ...
            src = 'prev'
        elif self.statechange is not None:
            src = '<state>.stateChange(%s)' % self.statechange
        elif self.contentchange is not None:
            src = '<smil>.contentControlChange'
            if self.contentchange != 'any':
                src = src + '(%s)' % self.contentchange
    ...
    def copy(self, uidremap):
        ...
        return MMSyncArc(dstnode, action, srcnode,
                         ...
                         self.accesskey, self.statechange, self.contentchange,
                         self.delay, self.implicit)

    def refnode(self):
        node = self.dstnode
        pnode = node.GetSchedParent()
        if self.wallclock is not None or \
               self.channel is not None or \
               self.accesskey is not None or \
               self.statechange is not None or \
               self.contentchange is not None:
...
##         if self.statechange is not None:
##             return self.dstnode.GetSchedRoot().eventhappened
...
##                 sctx.sched_arc(self, arc, curtime, event=event, marker=arc.marker, timestamp=self.happenings[key])
    ...
    def GetClip(self, attr, units):
        ...
        if self.context.state is not None:
            val = self.context.state.interpolate(val)
        if clipre is None:
            import re
            clipre = re.compile(
                    '^(?:'
...
##             print
    ...
    # this method return the channel type according to the node type
    # and mime type defined for this node.
    ...
    def guessChannelType(self, type = None, computedMimeType = None, url = None):
        if type is None:
            type = self.type
        if type in ('brush', 'animate', 'prefetch', 'state'):
            # for now, direct mapping
            return type
        if type in interiortypes:
            return None
        if computedMimeType is None:
    ...
    # and if stateList is not None, fill it with state nodes
    def GetAllMediaNodes(self, list = None, animateList = None, stateList = None):
        if list is None:
            list = []
        if self.type in mediatypes:
            list.append(self)
        ...
        elif stateList is not None and self.type == 'state':
            stateList.append(self)
        for node in self.children:
            node.GetAllMediaNodes(list, animateList, stateList)
        return list
    ...
    def GetComputedMimeType(self, url = None):
        if self.computedMimeType is None and self.type in ('imm', 'ext'):
            if url is None:
                url = self.attrdict.get('file')
                if self.context.baseurl:
                    url = MMurl.basejoin(self.context.baseurl, url)
...
##             print 'func is', child._is_realpix_with_captions(), child._is_realpix_with_captions
    ...
    def __calcduration(self, sctx):
        ...
        if self.type == 'animate':
            # intrinsic duration of an animate element is indefinite
            return -1, 1
        if self.type == 'state':
            # intrinsic duration of a state element is 0
            return 0, 1
        return 0, 0
    ...
    def ShouldPlay(self):
        ...
        for u_group in self.GetAttrDef('u_group', []):
            ...
            if val is not None and val[1] != 'RENDERED':
                return 0
        if self.attrdict.has_key('expr'):
            expr = self.attrdict['expr']
            if expr:
                e = self.context.state.expr(expr)
                if e == False:
                    return 0
        # Else we should
        self.shouldplay = 1
