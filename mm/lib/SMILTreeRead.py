...
stateeventre = re.compile(
    r'(?P<state>stateChange\((?P<ref>[^)]*)\))$' '|'
    r'(?P<content>contentControlChange(?:\((?P<attrname>' + xmllib._Name + r')\))?)$'
    )
...
class SMILParser(SMIL, xmllib.XMLParser):
    ...
    __enumattrs = {
        ...
        'backgroundRepeat': ['inherit', 'noRepeat', 'repeat', 'repeatX', 'repeatY'],
        ...
        'fit': ['meet', 'meetBest', 'slice', 'fill', 'hidden', 'scroll'],
        'immediate-instantiation': __truefalse,
        'immediate-instantiation': __truefalse,
        'mediaAlign': ['topLeft', 'topMid', 'topRight', 'midLeft', 'center', 'midRight', 'bottomLeft', 'bottomMid', 'bottomRight'],
        'method': ['get', 'put', 'post'],
        'mode': ['in', 'out'],
    ...
    def __init__(self, context, printfunc = None, new_file = 0, check_compatibility = 0, progressCallback=None):
        self._elements = {
                ...
                'paramGroup': (self.start_paramGroup, self.end_paramGroup),
                'state': (self.start_state, self.end_state),
                ...
                'setvalue': (self.start_setvalue, self.end_setvalue),
                'delvalue': (self.start_delvalue, self.end_delvalue),
                'newvalue': (self.start_newvalue, self.end_newvalue),
                'send': (self.start_send, self.end_send),
                'submission': (self.start_submission, self.end_submission),
        ...
        self.__seen_smil = 0
        self.__in_smil = 0
        self.__smil_id = None           # xml:id of <smil> element
        self.__in_head = 0
        ...
        self.__in_state = 0
        self.__state = []
        self.__stateurl = None
        self.__state_id = None          # xml:id of <state> element
        self.__submissions = {}
        self.__errorList = []
        self.__viewinfo = []
        ...
        for chtype in ChannelMap.SMIL2Channeltypes + ChannelMap.SMIL3Channeltypes:
            self.__validchannels[chtype] = 1
    ...
    def close(self):
        ...
        if self.__context.smilversion < 20:
            self.__context.smilversion = 20
    ...
    def SyncArc(self, node, attr, val):
        ...
        if res is not None:
            ...
            for val in vals:
                ...
                xanchor = xchan = statechange = contentchange = None
                res = xpathre.match(name)
                if res is not None:
                    xnode = res.group('xpath') # GRiNS extension
                else:
                    res = stateeventre.match(event)
                    if res is not None:
                        if not settings.MODULES['UserState']:
                            self.warning('ignoring event value %s in unsupported module' % event)
                            continue
                        if res.group('state'):
                            if self.__state_id is None or name != self.__state_id:
                                self.warning('stateChange event only raised by <state> element')
                                continue
                            statechange = res.group('ref')
                        elif res.group('content'):
                            if self.__smil_id is None or name != self.__smil_id:
                                self.warning('contentControlChange event only raised by <smil> element')
                                continue
                            contentchange = res.group('attrname')  # may be None
                        xnode = self.__root  # not the real source
                        event = None
                    else:
                        xnode = self.__nodemap.get(name)
                if xnode is None:
                    ...
                    if xchan is None:
                        ...
                        continue
                if (event is None and (statechange is not None or contentchange is not None)) or \
                       (event[:7] == 'repeat(' and settings.MODULES['RepeatValueTiming']) or \
                       (event in ('begin', 'end') and settings.MODULES['SyncbaseTiming']) or \
                       (event[:7] not in ('repeat(', 'begin', 'end') and settings.MODULES['EventTiming']):
                    list.append(
                        MMNode.MMSyncArc(node, attr, srcnode = xnode,
                                         channel = xchan, event = event,
                                         statechange = statechange,
                                         contentchange = contentchange,
                                         delay = offset or 0))
                continue
        if boston:
            if self.__context.smilversion == 10:
    ...
    def AddTestAttrs(self, attrdict, attributes):
        ...
        for attr in ('system-bitrate', 'systemBitrate'):
            if attributes.has_key(attr):
                ...
                del attributes[attr]
                boston = '-' not in attr
                if boston and self.__context.smilversion == 10:
                    self.syntax_error('%s attribute not compatible with SMIL 1.0' % attr)
                if boston and features.editor and self.__context.smilversion < 20:
                    self.__context.smilversion = 20
                if not boston or self.__context.smilversion >= 20:
                    try:
                        bitrate = int(val)
                    except ValueError:
        ...
        if attributes.has_key('systemAudioDesc'):
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'systemAudioDesc')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
        ...
        if attributes.has_key('systemCPU'):
            ...
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'systemCPU')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
        ...
        if attributes.has_key('systemOperatingSystem'):
            ...
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'systemOperatingSystem')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
        ...
        if attributes.has_key('systemOverdubOrSubtitle'):
            ...
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'systemOverdubOrSubtitle')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
                ...
                if not boston or self.__context.smilversion >= 20:
                    ...
                    if not list or (len(list) == 1 and list[0] == ''):
                        ...
                        if self.__context.smilversion == 10:
                            ...
                            if not features.editor:
                                list = []
                        elif self.__context.smilversion < 20:
                            self.__context.smilversion = 20
                    val = []
                    for v in list:
                        nsuri = nsdict.get(v)
        ...
        if attributes.has_key('systemComponent'):
            ...
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'systemComponent')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
        ...
        if attributes.has_key('customTest'):
            ...
            if self.__context.smilversion == 10:
                self.syntax_error('%s attribute not compatible with SMIL 1.0' % 'customTest')
            if features.editor and self.__context.smilversion < 20:
                self.__context.smilversion = 20
            if self.__context.smilversion >= 20:
                attrdict['u_group'] = []
        ...
        if attributes.has_key('expr'):
            val = attributes['expr']
            del attributes['expr']
            if 10 <= self.__context.smilversion < 30:
                self.syntax_error('%s attribute not compatible with SMIL %g' % ('expr', smil.__context.smilversion / 10.))
            if features.editor and self.__context.smilversion < 30:
                self.__context.smilversion = 30
            if self.__context.smilversion >= 30:
                attrdict['expr'] = val

    def AddAnimateAttrs(self, attrdict, attributes):
        if self.__context.smilversion == 10 and not features.editor:
    ...
    def AddAttrs(self, node, attributes):
        ...
        if node.type == 'animate':
            ...
            self.AddStateAttrs(node, attributes)
    ...
    def __do_fit(self, node, attr, val, attrdict):
        if val == 'meetBest' and self.__context.smilversion < 21:
            self.syntax_error('%s attribute value %s not compatible with SMIL %d.0' % (attr, val, self.__context.smilversion / 10))
            if not features.editor:
                return
            self.__context.smilversion = 21
    ...
    def __do_enumSMIL21(self, node, attr, val, attrdict):
        if self.__context.smilversion < 21:
            self.syntax_error('%s attribute not compatible with SMIL %d.0' % (attr, self.__context.smilversion / 10))
            if not features.editor:
                return
        if self.__context.smilversion < 21:
            self.__context.smilversion = 21
    ...
    def __do_enumSMIL30(self, node, attr, val, attrdict):
        if self.__context.smilversion < 30:
            self.syntax_error('%s attribute not compatible with SMIL %d.0' % (attr, self.__context.smilversion / 10))
            if not features.editor:
                return
    ...
    def __do_paramGroup(self, node, attr, val, attrdict):
        if self.__context.smilversion < 21:
            self.syntax_error('%s attribute not compatible with SMIL %d.0' % (attr, self.__context.smilversion / 10))
            if not features.editor:
                return
        ...
        if res is None or res.end(0) != len(val):
            self.syntax_error("illegal paramGroup value `%s'" % val)
        # paramGroup is allowed only a single value, but we're
        # prepared for more by using a list
        attrdict['paramgroup'] = [val]
    ...
    def NewNode(self, tagname, attributes):
        ...
        xmlid = self.__checkid(attributes)
        ...
        if tagname == 'brush':
            ...
            url = attributes.get('src') or None
        data = None
        mtype = None
        nodetype = 'ext'
        if url == '#':
...
##                     self.warning("file type doesn't match element", self.lineno)
        ...
        # XXX The background color shouldn't be set by default from the
        # XXX parser: deal with inherit value. This part should be cleaned
        # XXX at some point, but you'll have to take into account that the
        # XXX bgcolor code is not localized, and we also have to keep
        # XXX compatibility with the G2 version, ...
...
##         node.char_positions = (self.lineno, self.lineno+1)
        ...
        # but at some point, we should leave MMNode determinate itself
        # the right channel type.
#               node.SetChannelType(chtype)
    ...
    def __newTopRegion(self):
        if self.__rootLayout is None:
            ...
            xmlid = self.__mkid('viewport')
            self.__rootLayout = self.__context.newviewport(xmlid, -1, 'layout')
    ...
    def EndNode(self):
        ...
        if ChannelMap.isvisiblechannel(mtype):
            ...
            for attr in ['regAlign', 'regPoint', 'fit']:
                if node.attrdict.has_key(attr):
                    attrList.append((attr, node.attrdict[attr]))
            if node.attrdict.has_key('mediaAlign'):
                val = node.attrdict['mediaAlign']
                if not node.attrdict.has_key('regAlign'):
                    attrList.append(('regAlign', val))
                if not node.attrdict.has_key('regPoint'):
                    attrList.append(('regPoint', val))
            cssResolver.setRawAttrs(mediaCssId, attrList)

        for attr, attr2, attr3 in (('clipBegin', 'clip-begin', 'clipbegin'), ('clipEnd', 'clip-end', 'clipend')):
            clipval = attributes.get(attr)
            if clipval:
                if self.__context.smilversion == 10:
                    self.syntax_error('%s attribute not compatible with SMIL 1.0' % attr)
                    if not features.editor:
            ...
            if clipval:
                if '{' in clipval:
                    if self.__context.smilversion < 30:
                        self.syntax_error('interpolation syntax not compatible with SMIL %g' % self.__context.smilversion / 10.)
                        if features.editor:
                            self.__context.smilversion = 30
                        else:
                            clipval = None
                    if clipval:
                        if not settings.MODULES['StateInterpolation']:
                            clipval = None
                    if clipval:
                        node.attrdict[attr3] = clipval
                        clipval = None
            if clipval:
                res = clip.match(clipval)
                if res is not None:
                    if res.group('clock') and self.__context.smilversion < 20:
                        self.syntax_error('invalid %s attribute; should be "npt=<time>"' % attr)
                        clipval = None
                    ...
                    self.syntax_error('invalid %s attribute' % attr)
            if clipval:
                node.attrdict[attr3] = ''.join(clipval.split())
        if self.__in_a:
            # deal with hyperlink
            href, attrdict, xmlid = self.__in_a[:-1]
            if xmlid is not None and not self.__idmap.has_key(xmlid):
                self.__idmap[xmlid] = node.GetUID()
...
#               self.__context.addinternalchannels( [(chname, 'animate', node.attrdict), ] )
    ...
    def NewStateNode(self, tagname, attributes):
        xmlid = self.__checkid(attributes)
        if not self.__in_smil:
            self.syntax_error('node not in smil')
            return
        ...
        if self.__node:
            # the warning comes later from xmllib
            self.EndNode()
        for attr in self.__state_required_attrs[tagname]:
            if not attributes.has_key(attr):
                self.syntax_error('required attribute %s missing in element %s' % (attr, tagname))

        # create the node
        if not self.__root:
            # "can't happen"
            node = self.MakeRoot('state')
        elif not self.__container:
            self.syntax_error('node not in container')
            return
        else:
            node = self.__context.newnode('state')
            self.__container._addchild(node)
        node.__statetag = tagname
        self.AddAttrs(node, attributes)
        node.attrdict['stag'] = tagname
        node.attrdict['mimetype'] = 'state/%s' % tagname
        del node.__statetag
        self.__container = node

    def AddStateAttrs(self, node, attributes):
        for attr in self.__state_required_attrs[node.__statetag]:
            if attributes.has_key(attr):
                node.attrdict['state_%s' % attr] = attributes[attr]
                del attributes[attr]
            else:
                self.syntax_error('required attribute %s missing in element %s' % (attr, node.__statetag))
                node.attrdict['state_%s' % attr] = ''
        for attr in self.__state_optional_attrs[node.__statetag]:
            if attributes.has_key(attr):
                attrval = attributes[attr]
                if self.__enumattrs.has_key(attr):
                    attrval = self.parseEnumValue(attr, attrval)
                if attrval is not None:
                    node.attrdict['state_%s' % attr] = attrval
                del attributes[attr]

    def EndStateNode(self):
        self.__container = self.__container.GetParent()
...
##             ch['height'] = height
    ...
    __keep_ns = {GRiNSns:0, QTns:0, RP9ns:0, ITSns:0, XHTMLns:0}
    __smil2_ns = {}
    for __x in SMIL20ns + SMIL21ns + SMIL30ns:
        __smil2_ns[__x] = 0
    del __x
    def __fix_attributes(self, ns, tagname, attributes):
        ...
        if ns:
            if self.__keep_ns.has_key(ns): # ns in (GRiNSns, QTns, RP9ns, ITSns, XHTMLns)
                pass
            elif self.__smil2_ns.has_key(ns): # ns in SMIL20ns+SMIL21ns+SMIL30ns
                ns = ''
            else:
                mod = ''
                for sns in SMIL20ns + SMIL21ns:
                    if ns[:len(sns)] == sns:
                        mod = ns[len(sns):]
                        ns = ''
                        break
        ...
        for key, val in attributes.items():
            ...
            if len(nsattr) == 2:
                ans, attr = nsattr
                if self.__keep_ns.has_key(ans): # ans in (GRiNSns, QTns, RP9ns, ITSns, XHTMLns)
                    pass
                elif self.__smil2_ns.has_key(ans): # ans in SMIL20ns+SMIL21ns+SMIL30ns
                    ans = ''
                else:
                    mod = ''
                    for sns in SMIL20ns + SMIL21ns:
    ...
    def __checkid(self, attributes, defaultPrefixId='id', checkid = 1):
        ...
        xmlid = attributes.get('xml:id', attributes.get('id'))
        if xmlid is not None:
            if attributes.has_key('xml:id') and attributes.has_key('id'):
                self.syntax_error('xml:id and id not both allowed on one element')
            if checkid:
                res = xmllib.tagfind.match(xmlid)
                if res is None or res.end(0) != len(xmlid):
                    self.syntax_error("illegal ID value `%s'" % xmlid)
            if self.__ids.has_key(xmlid):
                self.syntax_error('non-unique id %s' % xmlid)
                xmlid = self.__mkid(defaultPrefixId)
            self.__ids[xmlid] = 0
        return xmlid
    ...
    def start_smil(self, attributes):
        ...
        if ns is None or ns == SMIL10:
            self.__context.smilversion = 10
        elif ns in SMIL20ns:
            self.__context.smilversion = 20
            settings.switch_profile(settings.SMIL_20_MODULES)
        elif ns in SMIL21ns:
            self.__context.smilversion = 21
            settings.switch_profile(settings.SMIL_21_MODULES)
        elif ns in SMIL30ns:
            self.__context.smilversion = 30
            settings.switch_profile(settings.SMIL_30_LANGUAGE_MODULES)
        if self.__context.smilversion == 10:
    ...
    def end_smil(self):
        ...
        state = ''.join(self.__state).strip()
        ctx.state = MMState.MMState(ctx, state, self.__stateurl)
        if self.__viewinfo:
    ...
    def end_head(self):
        ...
        if self.__submissions:
            self.__context.addsubmissions(self.__submissions.items())
...
##             self.__base = content
    ...
    def start_region(self, attributes, checkid = 1):
        ...
        if self.__in_layout != LAYOUT_SMIL:
            ...
            return
        xmlid = self.__checkid(attributes, checkid = checkid)
        # experimental code for switch layout
        self.__elementindex = self.__elementindex+1

        if xmlid is None:
            xmlid = '_#internalid%d' % self.__elementindex
            if self.__ids.has_key(xmlid):
                # "cannot happen"
                self.syntax_error('non-unique id %s' % xmlid)
            self.__ids[xmlid] = 0
        ...
        ch = self.__context.newchannel(xmlid, -1, 'layout')
        cssAttrs = []
        ...
        for attr, val in attributes.items():
            if attr == 'id' or attr == 'xml:id':
                ...
                if res is None or res.end(0) != len(val):
                    ...
                    self.__regionnames[val].append(xmlid)
                    ch['regionName'] = val
                ...
                self.__do_fit(None, attr, val, ch)
                ...
                if val is not None:
                    ch[attr] = val
            elif attr == 'backgroundImage':
                if self.__context.smilversion < 21:
                    self.syntax_error('%s attribute not compatible with SMIL %d.0' % (attr, self.__context.smilversion / 10))
                    if not features.editor:
                        return
                    self.__context.smilversion = 21
                ch['backgroundImage'] = val
            elif attr == 'backgroundRepeat':
                if self.__context.smilversion < 21:
                    self.syntax_error('%s attribute not compatible with SMIL %d.0' % (attr, self.__context.smilversion / 10))
                    if not features.editor:
                        return
                    self.__context.smilversion = 21
                val = self.parseEnumValue(attr, val)
                ch['backgroundRepeat'] = val
            else:
                # catch all
                ch[attr] = val
        if regAlign is not None:
            cssAttrs.append(('regAlign', regAlign))
        if regPoint is not None:
            cssAttrs.append(('regPoint', regPoint))
    ...
    def start_root_layout(self, attributes):
        ...
        xmlid = self.__checkid(attributes,'viewport')
        if xmlid is None:
            xmlid = self.__mkid('viewport')
        ...
        if self.__rootLayout is not None:
            ...
            if features.editor and features.MULTIPLE_TOPLAYOUT not in features.feature_set and len(self.__context.getviewports()) > 0:
                self.unsupportedfeature_error("multiple top layouts")
            topLayout = self.__context.newviewport(xmlid, -1, 'layout')
        ...
        for attr,val in attributes.items():
            if attr in ('open', 'close'):
                ...
                if self.__context.smilversion < 21:
                    ...
                    self.__context.smilversion = 21
                val = self.parseEnumValue(attr, val)
                topLayout['backgroundRepeat'] = val
##             else:
##                 # catch all
##                 attrdict[attr] = val
        ...
        for attr, val in attributes.items():
            if attr == 'id' or attr == 'xml:id':
                ...
                if val is not None:
                    attrdict[attr] = val
        if xmlid is not None:
            self.__context.addRegpoint(xmlid, attrdict)
    ...
    def start_state(self, attributes):
        if __debug__:
            if parsedebug: print 'start state', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('state element not compatible with SMIL %g' % self.__context.smilversion / 10.)
        if self.__context.smilversion < 30:
            self.__context.smilversion = 30
        xmlid = self.__checkid(attributes)
        self.__state_id = xmlid
        self.setliteral()
        self.__in_state = 1
    ...
    def start_custom_test(self, attributes):
        ...
        self.__custom_tests[xmlid] = title, not not u_state, override or 'hidden', uid
    ...
    def start_Glayout(self, attributes):
        ...
        if regions is None:
            ...
            return
        self.__layouts[xmlid] = regions.split()
    ...
    def __fixendsync(self, node):
        ...
        if endsync is None:
            ...
            if res is None:
                ...
                xmlid = res.group('id')
            if self.__nodemap.has_key(xmlid):
                child = self.__nodemap[xmlid]
    ...
    def start_setvalue(self, attributes):
        if __debug__:
            if parsedebug: print 'start setvalue', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('setvalue element not compatible with SMIL %g' % self.__context.smilversion / 10.)
            if not features.editor:
                self.setliteral()
                return
        if self.__context.smilversion < 30:
            self.__context.smilversion = 30
        self.NewStateNode('setvalue', attributes)

    def end_setvalue(self):
        if __debug__:
            if parsedebug: print 'end setvalue'
        if self.__context.smilversion < 30:
            return
        self.EndStateNode()

    def start_newvalue(self, attributes):
        if __debug__:
            if parsedebug: print 'start newvalue', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('newvalue element not compatible with SMIL %g' % self.__context.smilversion / 10.)
            if not features.editor:
                self.setliteral()
                return
        if self.__context.smilversion < 30:
            self.__context.smilversion = 30
        self.NewStateNode('newvalue', attributes)

    def end_newvalue(self):
        if __debug__:
            if parsedebug: print 'end newvalue'
        if self.__context.smilversion < 30:
            return
        self.EndStateNode()

    def start_delvalue(self, attributes):
        if __debug__:
            if parsedebug: print 'start delvalue', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('delvalue element not compatible with SMIL %g' % self.__context.smilversion / 10.)
            if not features.editor:
                self.setliteral()
                return
        if self.__context.smilversion < 30:
            self.__context.smilversion = 30
        self.NewStateNode('delvalue', attributes)

    def end_delvalue(self):
        if __debug__:
            if parsedebug: print 'end delvalue'
        if self.__context.smilversion < 30:
            return
        self.EndStateNode()

    def start_send(self, attributes):
        if __debug__:
            if parsedebug: print 'start send', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('send element not compatible with SMIL %g' % self.__context.smilversion / 10.)
            if not features.editor:
                self.setliteral()
                return
        if self.__context.smilversion < 30:
            self.__context.smilversion = 30
        self.NewStateNode('send', attributes)
        s = self.__container.attrdict.get('submission')
        if s is not None:
            if not self.__submissions.has_key(s):
                self.syntax_error('invalid submission specified in submission attributes')

    def end_send(self):
        if __debug__:
            if parsedebug: print 'end send'
        if self.__context.smilversion < 30:
            return
        self.EndStateNode()

    def start_submission(self, attributes):
        if __debug__:
            if parsedebug: print 'start submission', attributes
        if 10 <= self.__context.smilversion < 30:
            self.syntax_error('submission element not compatible with SMIL %g' % self.__context.smilversion / 10.)
            if not features.editor:
                self.setliteral()
                return
        if self.__context.smilversion < 30:
        ...
        for name, value in attributes.items():
            ...
            del attributes[name]
            dict[name] = value
        self.__submissions[xmlid] = dict

    def end_submission(self):
        if __debug__:
            if parsedebug: print 'end submission'
        pass
    ...
    def start_a(self, attributes):
        ...
        self.__in_a = href, attrdict, xmlid, self.__in_a
    ...
    def start_paramGroup(self, attributes):
        if __debug__:
            if parsedebug: print 'start paramGroup', attributes
        if self.__context.smilversion < 21:
            self.syntax_error('paramGroup not compatible with SMIL %d.0' % smil.__context.smilversion / 10)
            if not features.editor:
                self.setliteral()
                return
            self.__context.smilversion = 21
        xmlid = self.__checkid(attributes)
        if xmlid is None:
            xmlid = self.__mkid('paramGroup')
        self.__in_paramGroup = xmlid
        self.__paramGroups[xmlid] = {}

    def end_paramGroup(self):
        if __debug__:
            if parsedebug: print 'end paramGroup'
        if self.__context.smilversion < 21:
            return
        self.__in_paramGroup = None
    ...
    def start_param(self, attributes):
        ...
        if vtype != 'data':
            return
        if self.__in_paramGroup is not None:
            if self.__paramGroups[self.__in_paramGroup].has_key(name):
                self.syntax_error('param name %s already exists in paramGroup %s' % (name, self.__in_paramGroup))
            self.__paramGroups[self.__in_paramGroup][name] = value
            self.__do_param(name, value, checkonly = 1)
        else:
            self.__do_param(name, value)

    def __do_param(self, name, value, checkonly = 0):
        if name == 'color':
            fg = self.__convert_color(value, name)
            if not checkonly and fg is not None:
        ...
        if name == 'reliable':
            ...
            if not checkonly and reliable is not None:
    ...
    def handle_doctype(self, tag, pubid, syslit, data):
        ...
        if data:
            ...
            return
        if DOCTYPES.has_key(pubid):
            s, p, v = DOCTYPES[pubid]
            if syslit != s:
                self.warning('bad system literal in DOCTYPE')
            self.__context.smilversion = v
            self.__profile = p
            if v == 20:
                if p == 'Language':
                    settings.switch_profile(settings.SMIL_20_MODULES)
                ...
                if p == 'Language':
                    settings.switch_profile(settings.SMIL_21_MODULES)
                elif p == 'Extended Mobile':
                    settings.switch_profile(settings.SMIL_21_EXTENDED_MOBILE_MODULES)
                elif p == 'Mobile':
                    settings.switch_profile(settings.SMIL_21_MOBILE_MODULES)
            elif v == 30:
                if p == 'Language':
                    settings.switch_profile(settings.SMIL_30_LANGUAGE_MODULES)
                elif p == 'Extended Mobile':
                    settings.switch_profile(settings.SMIL_30_EXTENDED_MOBILE_MODULES)
                elif p == 'Mobile':
                    settings.switch_profile(settings.SMIL_30_MOBILE_MODULES)
                elif p == 'Tiny':
                    settings.switch_profile(settings.SMIL_30_TINY_MODULES)
                elif p == 'Daisy':
                    settings.switch_profile(settings.SMIL_30_DAISY_MODULES)
        else:
            self.syntax_error('unrecognized DOCTYPE')
    ...
    def finish_starttag(self, tagname, attrdict, method):
        ...
        nstag = tagname.split(' ')
        if len(nstag) == 2 and \
           (nstag[0] in [SMIL10]+SMIL20ns+SMIL21ns+SMIL30ns or settings.extensions.has_key(nstag[0])):
            ns, tagname = nstag
        else:
            ns = ''
        ...
        if len(self.stack) > 1:
            ...
            nstag = ptag.split(' ')
            if len(nstag) == 2 and \
               nstag[0] in [SMIL10]+SMIL20ns+SMIL21ns+SMIL30ns:
                pns, ptag = nstag
            else:
                pns = ''
        ...
        elif ns and ns == SMIL10:
            self.__context.smilversion = 10
        elif ns and ns in SMIL30ns:
            self.__context.smilversion = 30
            baseProfile = attrdict.get('baseProfile')
            if self.__profile is not None and baseProfile is not None and self.__profile != baseProfile:
                self.syntax_error('baseProfile and DOCTYPE do not match')
            if self.__profile is None:
                if baseProfile == 'Language':
                    settings.switch_profile(settings.SMIL_30_LANGUAGE_MODULES)
                elif baseProfile == 'Mobile':
                    settings.switch_profile(settings.SMIL_30_MOBILE_MODULES)
                elif baseProfile == 'ExtendedMobile':
                    settings.switch_profile(settings.SMIL_30_EXTENDED_MOBILE_MODULES)
                elif baseProfile == 'Tiny':
                    settings.switch_profile(settings.SMIL_30_TINY_MODULES)
                elif baseProfile == 'Daisy':
                    settings.switch_profile(settings.SMIL_30_DAISY_MODULES)
                else:
                    self.syntax_error('unknown profile')
        elif ns and ns not in SMIL20ns + SMIL21ns and ns[-8:] != 'Language':
            self.warning('default namespace should be "%s" or "%s"' % (SMIL20ns[0], SMIL21ns[0]), self.lineno)
...
class SMILMetaCollector(xmllib.XMLParser):
    ...
    def finish_starttag(self, tagname, attrdict, method):
        nstag = tagname.split(' ')
        if len(nstag) == 2 and \
           (nstag[0] in [SMIL10, GRiNSns]+SMIL20ns+SMIL21ns+SMIL30ns or settings.extensions.has_key(nstag[0])):
            ns, tagname = nstag
        else:
            ns = ''
        if self._elements.has_key(tagname):
    ...
    def __fix_attributes(self, ns, tagname, attributes):
        ...
        for key, val in attributes.items():
            ...
            if len(nsattr) == 2:
                ans, attr = nsattr
                if ans in (GRiNSns, QTns, RP9ns, ITSns, XHTMLns):
                    pass
                elif ans in SMIL20ns + SMIL21ns + SMIL30ns:
                    pass
                else:
                    for sns in SMIL20ns + SMIL21ns:
                        if ans[:len(sns)] == sns:
...
def _uniqname(namelist, defname):
    ...
    if defname is None:
        maxid = 0
        for xmlid in namelist:
            try:
                xmlid = eval('0+'+xmlid)
            except:
                pass
            if type(xmlid) is type(0) and xmlid > maxid:
                maxid = xmlid
        return `maxid+1`
    xmlid = 0
    while ('%s_%d' % (defname, xmlid)) in namelist:
        xmlid = xmlid + 1
    return '%s_%d' % (defname, xmlid)
