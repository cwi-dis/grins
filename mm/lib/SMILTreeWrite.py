...
doctype10 = '<!DOCTYPE smil PUBLIC "%s"\n%s"%s">\n' % (SMIL10Pubid,' '*22,SMIL10Dtd)
doctype20 = '<!DOCTYPE smil PUBLIC "%s"\n%s"%s">\n' % (SMIL20Pubid,' '*22,SMIL20Dtd)
...
class SMILWriter(SMIL, BaseSMILWriter, SMILWriterBase):
    def __init__(self, node, fp, filename, grinsExt = 1,
        ...
        if smil_one:
            ...
            self.smilboston = ctx.smilversion >= 20
    ...
    def write(self):
        ...
        if not self.uses_grins_namespace and not self.uses_rp_namespace and not self.uses_qt_namespace:
            if self.smilboston:
                self.writestring(doctype20)
            else:
                self.writestring(doctype10)
        if ctx.comment:
            self.writecomment(ctx.comment)
        attrlist = []
        if self.smilboston:
            attrlist.append(('xmlns', SMIL20ns[0]))
            ...
            if self.smilboston:
                ...
                if ch.has_key('close'):
                    ...
                    if val != 'onRequest':
                        ...
                        useRootLayout = 0
                if ch.has_key('backgroundImage'):
                    val = ch['backgroundImage']
                    if val != 'none':
                        attrlist.append(('backgroundImage', val))
                if ch.has_key('backgroundRepeat'):
                    val = ch['backgroundRepeat']
                    if val != 'repeat':
                        attrlist.append(('backgroundRepeat', val))
...
##                         attrlist.append(('%s:%s' % (NSGRiNSprefix, key), MMAttrdefs.valuerepr(key, val)))
    ...
    def writeregion(self, ch):
        ...
        if ChannelMap.isvisiblechannel(ch['type']):
            ...
            if fit not in ('meet','meetBest','slice','hidden','fill','scroll'):
                fit = None
