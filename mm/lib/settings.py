...
# HierarchyView colors
	...
	'templatedir_url': 'http://www.AmbulantPlayer.org/indir/images',
...
def set(setting, value, dontsave = 0):
	...
	if setting in NEEDS_RESTART and value != get(setting) and not _warned_already:
		_warned_already = 1
		windowinterface.showmessage('You have to restart Ambulant_G for some of these changes to take effect.')
	if dontsave:
...
def read_components_from_skin(skindict = None):
	...
	if skindict is None:
		url = get('skin')
		if not url:
			components = ['http://www.AmbulantPlayer.org/Ambulant_G/smil2.0']
			return
		try:
			u = MMurl.urlopen(url)
		except:
