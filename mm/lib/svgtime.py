...
class TimeElement(TimeNode, Timer):
	...
	def begin(self):
		self._active = 1
		self.tdocument.getTimeRoot().onBeginEvent(self)

	def end(self):
		self._active = 0
		self.tdocument.getTimeRoot().onEndEvent(self)
...
class SvgSyncArc:
	...
	def updateDependant(self, tval):
		self.attrobj.setSyncBaseValue(tval)
