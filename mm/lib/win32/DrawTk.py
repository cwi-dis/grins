...
class DrawTool:
	...
	def onLButtonDown(self,wnd,flags,point):
		wnd.SetCapture()
		wnd.down_flags = flags
		wnd.down = point
		wnd.last = point

	def onLButtonDblClk(self,wnd,flags,point):
		pass

	def onLButtonUp(self, wnd, flags, point):
		wnd.ReleaseCapture()
		if point.iseq(wnd.down):
			wnd.currentToolType = TOOL_SELECT

	def onMouseMove(self,wnd,flags,point):
		wnd.last = point
		cursor=Sdk.LoadStandardCursor(win32con.IDC_ARROW)
		wnd.SetCursor(wnd,cursor)

	# Cancel drawing
	def onCancel(self,wnd):
		wnd.drawwnd.currentToolType = TOOL_SELECT
...
class SelectTool(DrawTool):
	def __init__(self):
		DrawTool.__init__(self,TOOL_SELECT)
	...
	def onLButtonDown(self,wnd,flags,point):
		...
		if len(wnd._selection) == 1:
			...
			wnd.ixDragHandle = drawObj.hitTest(local,wnd,1)
			if wnd.ixDragHandle != 0:
				wnd.selectMode = SM_SIZE
				# for smoother behavior set cursor here
				wnd.SetCursor(wnd,drawObj.getHandleCursor(wnd.ixDragHandle))
		...
		wnd.lastPoint = local
		DrawTool.onLButtonDown(self,wnd,flags, point)
	...
	def onLButtonDblClk(self,wnd,flags,point):
		if (flags & win32con.MK_SHIFT) != 0:
			...
			if len(wnd._selection)==1:
				wnd._selection[0].onOpen(wnd)
		DrawTool.onLButtonDblClk(self,wnd,flags,point)
	...
	def onLButtonUp(self,wnd,flags,point):
		if wnd.MouseCaptured():
			if wnd.selectMode == SM_NET:
				...
				wnd.InvalidateRect()
				wnd.ReleaseDC(dc)
		DrawTool.onLButtonUp(self,wnd,flags,point)
	...
	def onMouseMove(self,wnd,flags,point):
		if not wnd.MouseCaptured():
			if wnd.currentToolType == TOOL_SELECT and len(wnd._selection) == 1:
				drawObj = wnd._selection[0]
				local=wnd.ClientToCanvas(point)
				nHandle = drawObj.hitTest(local,wnd, 1)
				if nHandle != 0:
					wnd.SetCursor(wnd,drawObj.getHandleCursor(nHandle))
					return # bypass DrawTool
			if wnd.currentToolType == TOOL_SELECT:
				DrawTool.onMouseMove(self,wnd,flags,point)
			return
		...
		for drawObj in wnd._selection:
			...
			if wnd.selectMode == SM_MOVE:
				...
				drawObj.moveTo(position,wnd)
				if delta.x+delta.y:wnd.SetDrawObjDirty(1)
			elif wnd.ixDragHandle != 0:
				drawObj.moveHandleTo(wnd.ixDragHandle,local,wnd)
...
class RectTool(DrawTool):
	def __init__(self):
		DrawTool.__init__(self,TOOL_RECT)
	...
	def onLButtonUp(self,wnd,flags,point):
		...
		wnd.selectTool.onLButtonUp(wnd,flags,point)
		wnd.OnNewRect(wnd)		
	...
	def onMouseMove(self,wnd,flags,point):
		...
		wnd.SetCursor(wnd,cursor)
		wnd.selectTool.onMouseMove(wnd,flags,point)
...
class DrawObj:
	...
	def moveTo(self,position,wnd):
		if position.iseq(self._position):
			return
		if not wnd.InDrawAreaRc(position):
			return
		if not wnd:
			self.invalidate()
	...
	def hitTest(self,point,wnd,is_selected):
		"""
		...
		if is_selected:
			...
			for nHandle in range(1,nHandleCount+1):
				...
				rc = self.getHandleRect(nHandle,wnd)
...
class DrawRect(DrawObj):
	...
	def moveHandleTo(self,ixHandle,point,wnd = None):
		return DrawObj.moveHandleTo(self,ixHandle,point,wnd)
	def intersects(self,rect):
	...
	def crcfill(self,dc,wnd, clr=(0,0,0)):
		cr = win32mu.RGB(clr)
		l,t,r,b = wnd._crect.tuple()
...
class DrawLayer:
	def __init__(self):
		...
		# tk section
		self.tool={TOOL_SELECT:SelectTool(),TOOL_RECT:RectTool()}
		self.down=Point((0,0))
		self.down_flags=0
		self.last=Point((0,0))
	...
	def onCancelEdit(self):
		...
		self.drawwnd.currentToolType = self.TOOL_SELECT
	...
	def SetCursor(self,wnd,cursor):
		if cursor!=Sdk.GetCursor():
			#Sdk.SetCursor(cursor)
			Sdk.SetClassLong(wnd.GetSafeHwnd(),win32con.GCL_HCURSOR,cursor)
	...
	def OnNewRect(self,wnd):
		if hasattr(self,'_limit_rect'):
			if len(wnd._objects)>=self._limit_rect:
				self.currentToolType=TOOL_SELECT	
