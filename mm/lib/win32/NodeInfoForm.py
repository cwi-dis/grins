...
class NodeInfoFormNEW(GenFormView):
	...
	def setdata(self):	
		...
		if self._name: self.setname(self._name)
		if self._immtext:
			self['ImmEdit'].settext(self._immtext)
		elif self._children:
			self['IntList'].setchildren(self._children)
		elif self._filename:
	...
	def ext_group_show(self):
		...
		self.GetParent().SetWindowPos(self.GetParent().GetSafeHwnd(),
			(0,0,rcDlg.width(), rc.bottom-rcDlg.top+sysmetrics.cycaption), 
			win32con.SWP_NOACTIVATE |win32con.SWP_NOMOVE| win32con.SWP_NOZORDER)
		self.SetWindowPos(self.GetSafeHwnd(),
			(0,0,rcDlg.width(), rc.bottom-rcDlg.top), 
			win32con.SWP_NOACTIVATE |win32con.SWP_NOMOVE| win32con.SWP_NOZORDER)
	...
	def RePosControls(self):
		rcDlg=win32mu.Rect(self.GetWindowRect())
		rcPromp=win32mu.Rect(self.GetDlgItem(grinsRC.IDC_STATIC1).GetWindowRect())	
		rc=win32mu.Rect(self.GetDlgItem(grinsRC.IDC_INT_LIST).GetWindowRect())

		h=rc.bottom-rcPromp.top;
		w=rc.width();
		x=rc.left-rcDlg.left;
		y=rcPromp.top-rcDlg.top-sysmetrics.cycaption-8;
