...
class Region(base_window.Window):
	...
	def hookOsWndMessages(self):
		...
		self.HookMessage(self.onOsWndLButtonDown,wincon.WM_LBUTTONDOWN)
		self.HookMessage(self.onOsWndLButtonUp,wincon.WM_LBUTTONUP)
		self.HookMessage(self.onOsWndMouseMove,wincon.WM_MOUSEMOVE)
	...
	def show(self):
		self._isvisible = 1
		if self._oswnd:
			self._oswnd.ShowWindow(wincon.SW_SHOW)
		self.update()

	def hide(self):
		self._isvisible = 0
		if self._oswnd:
			self._oswnd.ShowWindow(wincon.SW_HIDE)
		self.update()
	...
	def updateoswndpos(self):
		...
		if self._oswnd:
			x, y, w, h = self.getwindowpos()
			self._oswnd.SetWindowPos(wincon.HWND_TOP, (x,y,w,h) , 
				wincon.SWP_NOZORDER | wincon.SWP_ASYNCWINDOWPOS | wincon.SWP_DEFERERASE)
	...
	def getClipRgn(self, rel=None):
		...
		rgn.CombineRgn(rgn, prgn, wincon.RGN_AND)
		prgn.DeleteObject()
		return rgn
	...
	def getChildrenRgn(self, rel=None):
		...
		rgn.CombineRgn(rgn,argn,wincon.RGN_AND)
		argn.DeleteObject()
		return rgn
	...
	def getChildrenRgnComplement(self, rel=None):
		...
		rgn.CombineRgn(rgn,drgn,wincon.RGN_DIFF)
		drgn.DeleteObject()
		return rgn

	def clipRect(self, rc, rgn):
		newrgn = wingdi.CreateRectRgn(self.ltrb(rc))
		newrgn.CombineRgn(rgn,newrgn,wincon.RGN_AND)
	...
	def _paintOnDDS(self, dds, dst, rgn=None):
		...
		if self._active_displist:
			...
			if bgcolor:
				r, g, b = bgcolor
				dds.BltFill((xc, yc, xc+wc, yc+hc), bgcolor)
			if self._redrawdds:
				# get redraw dds info
			...
			dds.BltFill((xc, yc, xc+wc, yc+hc), self._bgcolor)
	...
	def getBackDDS(self, exclwnd = None, dopaint = 1):
		...
		try:
			dds.Blt((0,0,w,h), bf, (x, y, x+w, y+h))
		except ddraw.error, arg:
	...
	def bltDDS(self, srfc):
		...
		src_w, src_h = srfc.GetSize()
		rc_src = (0, 0, src_w, src_h)
		bf = self._topwindow.getDrawBuffer()
		if rc_dst[2]!=0 and rc_dst[3]!=0:
			try:
				bf.Blt(self.ltrb(rc_dst), srfc, rc_src)
			except ddraw.error, arg:
				print arg			
...
#			print 'normal paint while in transition', self 
	...
	def _paint_3(self, rc=None, exclwnd=None):
		...
		x, y, w, h = self.getwindowpos()
		try:
			dstDC.BitBlt((x, y),(w, h),srcDC,(0, 0), wincon.SRCCOPY)
		except ddraw.error, arg:
			print arg			
...
#				wnd._paint_0(rc, exclwnd)
	...
	def begintransition(self, outtrans, runit, dict, cb):
		...
		if runit:
			...
			self._transition = base_transitions.InlineTransitionEngine(self, outtrans, runit, dict, cb)
			self._transition.begintransition()
...
class ViewportContext:
	def __init__(self, wnd, w, h, units, bgcolor):
		...
		self._backBuffer = ddraw.CreateSurface(wnd, w, h)
		self._wnd.HookMessage(self.OnPaint, wincon.WM_PAINT)
	...
	def update(self, rc=None, exclwnd=None):
		...
		try:
			self._backBuffer.BltFill(rcPaint, self._bgcolor)
		except ddraw.error, arg:
			print arg
			return
	...
	def CreateSurface(self, w, h):
		dds = ddraw.CreateSurface(self._wnd, w, h)
		dds.BltFill((0, 0, w, h), self._bgcolor)
		return dds
...
def getcursorhandle(strid):
	return winuser.LoadStandardCursor(wincon.IDC_ARROW)
