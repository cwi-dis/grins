...
class ImageChannel(ChannelWindow):
	...
	def do_arm(self, node, same=0):
		...
		syncarm = self.syncarm
		try:
			if syncarm:
				result = MMurl.urlretrieve(f)
			else:
				MMurl.urlretrieve_async(f, self.do_arm_rest, node)
		except IOError, arg:
			...
			return 1
		if syncarm:
			self.do_arm_rest(node, result)
		return 0

	def do_arm_rest(self, node, result):
		f = result[0]
