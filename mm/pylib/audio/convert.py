...
class onechannel(audio_filter):
    def __init__(self, rdr, channel):
        fmt = rdr.getformat()
        print fmt
        audio_filter.__init__(self, rdr, fmt)
        if channel == 'left':
            ...
            self.__left = 0.0
            self.__right = 1.0
        self.__width = (fmt.getbps() + 7) / 8

    def readframes(self, nframes = -1):
        import audioop
        data, gotframes = self._rdr.readframes(nframes)
        data = audioop.tomono(data, self.__width, self.__left, self.__right)
        data = audioop.tostereo(data, self.__width, self.__left, self.__right)
        return data, gotframes
...
def convert(rdr, dstfmts = None, rates = None, loop = 1, channel = 'both'):
