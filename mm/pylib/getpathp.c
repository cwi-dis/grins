...
#if defined(MS_WIN32) && defined(IGNORE_REGISTRY)
char* getentry(char* pszSec,char* pszEntry, char *psz,int len)
	{
	char szBuf[MAX_PATH];
	char *p;
...
#if defined(MS_WIN32) && defined(IGNORE_REGISTRY)
	char homebuf[256]="";
	char *pythonhomei=getentry("GENERAL","PYTHONHOME",homebuf,256);
	if(pythonhomei[0]) {
		Py_SetPythonHome(pythonhomei);
		pythonhome = Py_GetPythonHome();
	}	
#endif
