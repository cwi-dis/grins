...
_translation = []
for i in range(256):
	_translation.append(chr(i ^ 128))
_translation = string.joinfields(_translation, '')
del i
...
## 		  _data_chunk, _framesize: private
	...
	def initfp(self, file, magicskipped = 0):
		self._file = file
		self._convert = None
		self._soundpos = 0
		if not magicskipped:
	...
	def __init__(self, f, magicskipped = 0):
		if type(f) == type(''):
			f = __builtin__.open(f, 'r')
		# else, assume it is an open file object already
		self.initfp(f, magicskipped)
...
## 	access *: private
	...
	def _read_fmt_chunk(self, chunk):
		...
		if wFormatTag == WAVE_FORMAT_PCM:
			...
			self._convert = lambda x: string.translate(x, _translation)
		else:
...
def open(f, mode, magicskipped = 0):
	if mode == 'r':
		return Wave_read(f, magicskipped)
	elif mode == 'w':
		return Wave_write(f)
	else:
