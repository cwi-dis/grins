...
BYTE * BMPFile::LoadBMP(String fileName, 
						UINT *width, 
						UINT *height)
{
	BITMAP inBM;
...
void BMPFile::SaveBMP(String fileName,		// output path
							BYTE * buf,				// BGR buffer
							UINT width,				// pixels
							UINT height)
{
...
BOOL BMPFile::BGRFromRGB(BYTE *buf, UINT widthPix, UINT height)
{
	if (buf==NULL)
		return FALSE;
...
BOOL BMPFile::VertFlipBuf(BYTE  * inbuf, 
					   UINT widthBytes, 
					   UINT height)
{   
	BYTE  *tb1;
...
inline mwrite(void* pb,UINT cb,UINT size,BYTE*& p)
   	{int n=cb*size;memcpy(p,pb,n);p += n;}

BYTE *BMPFile::SaveBMP2Mem( BYTE * buf,				// BGR buffer
							UINT width,				// pixels
							UINT height,
							DWORD *pdwSize)
{
	...
	*pdwSize=bmfsize;
	...
	mwrite((BYTE  *)&(m1),1,1,mp); byteswritten+=1;
	mwrite((BYTE  *)&(m2),1,1,mp); byteswritten+=1;
	mwrite((long  *)&(bmfsize),4,1,mp);	byteswritten+=4;
	mwrite((int  *)&(res1),2,1,mp); byteswritten+=2;
	mwrite((int  *)&(res2),2,1,mp); byteswritten+=2;
	mwrite((long  *)&(pixoff),4,1,mp); byteswritten+=4;

	mwrite((BITMAPINFOHEADER *)&header,sizeof(BITMAPINFOHEADER),1,mp);
	byteswritten+=sizeof(BITMAPINFOHEADER);
	
	long row=0;
	long rowidx;
	long row_size=header.biWidth*3;
	for (row=0;row<header.biHeight;row++) 
		{
		rowidx=(long unsigned)row*row_size;						      

		// write a row
		mwrite((void  *)(buf+rowidx),row_size,1,mp);
		byteswritten+=row_size;	
								 ...
								 RGBQUAD *colormap)			// array of RGBQUADs 
{
	...
	//AfxMessageBox("BMPFile::SaveBMP 1,4,8 bit BMP");
	m_errorText="OK";
	...
	fwrite(&res1, 2, 1, fp);
	fwrite(&res2, 2, 1, fp);
	fwrite(&pixeloffset, 4, 1, fp);
	fwrite(&bmisize, 4, 1, fp);
	fwrite(&cols, 4, 1, fp);
	fwrite(&rows, 4, 1, fp);
	fwrite(&planes, 2, 1, fp);
	fwrite(&bitsperpixel, 2, 1, fp);
	fwrite(&compression, 4, 1, fp);
	fwrite(&cmpsize, 4, 1, fp);
	fwrite(&xscale, 4, 1, fp);
	fwrite(&yscale, 4, 1, fp);
	fwrite(&colors, 4, 1, fp);
	fwrite(&impcolors, 4, 1, fp);
