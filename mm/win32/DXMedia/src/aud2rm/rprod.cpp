...
IRMAInputPin*   gInputPin = NULL;

// created 
IRMAMediaSample* gpMediaSample=NULL;
...
bool SetInputPin(IUnknown *p)
{
	PN_RELEASE(gInputPin);
	PN_RESULT res=p->QueryInterface(IID_IRMAInputPin,(void**)&gInputPin);
...
bool CreateMediaSample()
{
	...
	res = clfac->CreateInstance(CLSID_IRMAMediaSample, NULL, IID_IRMAMediaSample, (void **) &gpMediaSample);
	PN_RELEASE(clfac);
	if (!SUCCEEDED(res)) {
...
bool SetVideoInfo(int w,int h,float rate)
	{
	if(!gInputPin) return false;
	PN_RESULT res=PNR_OK;
	IRMAPinProperties* pUnkPinProps;
	IRMAVideoPinProperties* gInputPinProps;
	PN_CHECK(GetPinProperties,gInputPin->GetPinProperties(&pUnkPinProps));
	PN_CHECK(QueryInterface_IRMAVideoPinProperties,pUnkPinProps->QueryInterface(IID_IRMAVideoPinProperties,(void**)&gInputPinProps));
	PN_CHECK(SetVideoSize,gInputPinProps->SetVideoSize(w, h));
	PN_CHECK(SetVideoFormat,gInputPinProps->SetVideoFormat(ENC_VIDEO_FORMAT_RGB24));
	PN_CHECK(SetCroppingEnabled,gInputPinProps->SetCroppingEnabled(FALSE));
	PN_CHECK(SetFrameRate,gInputPinProps->SetFrameRate(rate));
	PN_RELEASE(gInputPinProps);
...
bool SetAudioInfo(int nchan,DWORD srate,int ssize)
	...
	PN_RESULT res=PNR_OK;
	IRMAPinProperties* pUnkPinProps;
	IRMAAudioPinProperties* gInputPinProps;
	PN_CHECK(GetPinProperties,gInputPin->GetPinProperties(&pUnkPinProps));
	PN_CHECK(QueryInterface_IRMAAudioPinProperties,pUnkPinProps->QueryInterface(IID_IRMAAudioPinProperties,(void**)&gInputPinProps));
	PN_CHECK(SetNumChannels,gInputPinProps->SetNumChannels(nchan));
	PN_CHECK(SetSampleRate,gInputPinProps->SetSampleRate(srate));
	PN_CHECK(SetSampleSize,gInputPinProps->SetSampleSize(ssize));
	PN_RELEASE(gInputPinProps);
	PN_RELEASE(pUnkPinProps);
...
bool EncodeSample(BYTE *p,DWORD size,DWORD msec,bool isSync,bool isLast)
	{
	if(!gpMediaSample) return false;

	UINT16	unFlags=0;
	if(isSync)unFlags|=MEDIA_SAMPLE_SYNCH_POINT;
	if(isLast)unFlags|=MEDIA_SAMPLE_END_OF_STREAM;
	PN_RESULT res=PNR_OK;
	res = gpMediaSample->SetBuffer(p,size,msec,unFlags);
	if(SUCCEEDED(res))
		{
		res=gInputPin->Encode(gpMediaSample);
		if(SUCCEEDED(res))
			Log("Sample encoded\n");
		else
			Log("Failed to encode sample\n");
		}
	else 
		Log("Failed to set buffer\n");
	return SUCCEEDED(res)!=FALSE;
	}
