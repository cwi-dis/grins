...
static PyObject* py_example_CreateFileOpenDlg(PyObject *self, PyObject *args)
{
	...
	CString filename, directory, fullpath, filter,
			tmp,ext,extensions;
	OPENFILENAME ofn;
	char *wndName, *fname, *dir, *fltr;
	int counter = 0;
	DWORD style;
	BOOL id;

	if(!PyArg_ParseTuple(args, "ssss", &wndName, &fname, &dir, &fltr))
		return NULL;

	style = OFN_FILEMUSTEXIST|OFN_HIDEREADONLY;

	tmp = fltr;
	while(tmp.Find('(')>=0)
	{
		while(tmp.Find(' ')==0) tmp = tmp.Mid(1);
		if(tmp.Find("*.")>=0)
		{
			int x = tmp.Find(')');
			if(x>=0)
			{
				ext = tmp.Mid(tmp.Find('(')+1,x-tmp.Find('(')-1);
				if(ext.Find('.')==-1) break;
				extensions += ext+", ";
				filter += tmp.Mid(0,x+1)+'\0'+ext+'\0';
				tmp = tmp.Mid(x+1);
				counter++;
			}
			else break;
	...
	//if (strcmp(fname," ")==0)
		ofn.lpstrFile = "\0";
	//else ofn.lpstrFile = (LPTSTR)fname; 
	ofn.nMaxFileTitle = _MAX_FNAME + _MAX_EXT;
	ofn.lpstrFileTitle = NULL;
	//if (strcmp(dir," ")==0)
		ofn.lpstrInitialDir = "\0";
	//else ofn.lpstrInitialDir =(LPCTSTR) dir; 
	ofn.lpstrTitle = (LPCTSTR)wndName; 
	ofn.Flags = style; 
	ofn.nFileOffset = 0; 
	...
	if(!id)
	{
		TRACE("CmifEx CreateFileOpenDlg FALSE!\n");
		testOb = Py_BuildValue("isss", (UINT)id, (LPCTSTR)filename, (LPCTSTR)directory, (LPCTSTR)ext);
		return testOb;
	}	
  
				
	filename = ofn.lpstrFile;
	directory = ofn.lpstrInitialDir;
	ext = filename.Mid(ofn.nFileExtension);
	if((ofn.nFileExtension!=0)&&(!ext.IsEmpty()))
			ext = "*."+ext;
	else ext = "*. ";
	
	if(extensions.Find("*.*")==-1)
	{
		if(extensions.Find((LPCTSTR)ext)==-1)
		{
			id = FALSE;
			CString msg = "File: "+filename+'\n'+"has not one of the \n"+extensions+" extensions"+'\n'+"and can not be read.";
			MessageBox(GetFocus(),(LPCTSTR)msg,"Warning !",MB_OK|MB_ICONEXCLAMATION);
			filename = directory = ext = "";
		}
	}
...
static PyObject* py_example_CreateFileSaveDlg(PyObject *self, PyObject *args)
{
	...
	style = OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY;
