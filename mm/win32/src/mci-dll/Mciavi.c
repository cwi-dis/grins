...
void positionMovie_old(MCI_AVI_STRUCT *mciAviInfo)
{
	...
	{
		rcMovie.left = rcClient.left; 
		rcMovie.top = rcClient.top; 
		rcMovie.right = rcClient.right; 
		rcMovie.bottom = rcClient.bottom; 

		/* reposition the playback (child) window */
		MoveWindow(mciAviInfo->hWndMovie, rcMovie.left, rcMovie.top,
					rcMovie.right, rcMovie.bottom, TRUE);
	}
...
void positionMovie(MCI_AVI_STRUCT *mciAviInfo)
{
	...
	if (!mciAviInfo->fStretch) //don't stretch
	{
		...
		/* reposition the playback (child) window */
		MoveWindow(mciAviInfo->hWndMovie, winrc.left, winrc.top-GetSystemMetrics(SM_CYEDGE),
					(winrc.right-winrc.left), (winrc.bottom-winrc.top), TRUE);
