...
long MidiClip( MCI_MIDI_STRUCT *mciMidiInfo ,long clip)
{
    MCI_STATUS_PARMS mciStatus;
    MCI_SET_PARMS mciSet;

    mciSet.dwTimeFormat = MCI_FORMAT_FRAMES;
    mciSendCommand(mciMidiInfo->wMIDIDeviceID,
					MCI_SET, MCI_SET_TIME_FORMAT,
					(DWORD)(LPMCI_STATUS_PARMS)&mciSet);

    mciMidiInfo->iSeekThere = clip;
    MidiSeek( mciMidiInfo) ; 
...
DWORD MidiDuration(MCI_MIDI_STRUCT *mciMidiInfo)
{
