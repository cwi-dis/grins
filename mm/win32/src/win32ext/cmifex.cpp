...
IMPLEMENT_PYMODULECLASS(Cmifex,GetCmifex,"Cmifex Module Wrapper Object");
/////////////////
...
static PyObject* py_cmifex_FillPolygon(PyObject *self, PyObject *args)
{
	...
	if((pts = (POINT*)malloc(npts_list*sizeof(POINT)))==NULL)
		RETURN_ERR("FillPolygon: Not Enough Memory!");
	...
	hCWnd = GetWndPtr( ob );	
	if((hdc = GetDC(hCWnd->m_hWnd))==NULL)
		RETURN_ERR("FillPolygon: NULL HDC!");
	
	if((hBrush = (HBRUSH)SelectObject(hdc, CreateSolidBrush (RGB(r, g, b))))==NULL)
		RETURN_ERR("FillPolygon: NULL BRUSH!");

	if((hPen = (HPEN)SelectObject(hdc, CreatePen(PS_SOLID, 0, RGB(r, g, b))))==NULL)
		RETURN_ERR("FillPolygon: NULL PEN!");
...
static PyObject* py_cmifex_DrawLines(PyObject *self, PyObject *args)
{
	...
	if((pts = (POINT*)malloc(npts_list*sizeof(POINT)))==NULL)
	{
		RETURN_ERR("DrawLines: Not Enough Memory!");
	}
	...
	hCWnd = GetWndPtr( ob );	
	if((hdc = GetDC(hCWnd->m_hWnd))==NULL)
		RETURN_ERR("DrawLines: NULL HDC!");
	
	if((hPen = (HPEN)SelectObject(hdc, CreatePen(PS_SOLID, 0, RGB(r, g, b))))==NULL)
		RETURN_ERR("DrawLines: NULL PEN!");
...
static PyObject* py_example_PrepareText(PyObject *self, PyObject *args)
{
	...
	if(!PyArg_ParseTuple(args, "OOisisis", 
		&testOb, &callback, &id, &filename, &inputType, &facename, &size, &algn))
		{
		return NULL;
		}
	...
	if(id==-1)
	{
		...
		if (!PyCallable_Check (callback))
			RETURN_ERR("Bad callback function");
	...
	if(inputType)
	{
		if(!f.Open(filename, CFile::modeRead, NULL))
			RETURN_ERR("Cannot open the file");
		else
		{	
...
DEFINE_PYMODULETYPE("PyCmifex",Cmifex)
