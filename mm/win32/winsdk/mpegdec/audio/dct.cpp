...
int mpeg2audio_dct64_1(float *out0, float *out1, float *b1, float *b2, float *samples)
{
	register float *costab = mpeg2_pnts[0];
	...
	{
		register float const cos0 = mpeg2_pnts[3][0];
  		register float const cos1 = mpeg2_pnts[3][1];
 	...
 	{
		register float const cos0 = mpeg2_pnts[4][0];
...
{
	float bufs[0x40];
	return mpeg2audio_dct64_1(a, b, bufs, bufs + 0x20, c);
}
...

	{
...

    	{
    		...
    		{ 
    			...
    			t0 = mpeg2_COS6_2 * (in[8] + in[16] - in[4]);
    			t1 = mpeg2_COS6_2 * in[12];
    			...
    			t2 = mpeg2_COS6_1 * (in[10] + in[14] - in[2]);
    			tmp[1] -= t2;
    			tmp[7] += t2;
    		}
    		{
    			...
    			t0 = mpeg2_cos9[0] * (in[4] + in[8] );
    			t1 = mpeg2_cos9[1] * (in[8] - in[16]);
    			t2 = mpeg2_cos9[2] * (in[4] + in[16]);
    	...
    	}
    	{
    		...
    		t1 = mpeg2_cos18[0] * (in[2]  + in[10]);
    		t2 = mpeg2_cos18[1] * (in[10] - in[14]);
    		t3 = mpeg2_COS6_1   * in[6];
...

    	{
    		...
    		t1 = mpeg2_COS6_2 * in[13];
    		t2 = mpeg2_COS6_2 * (in[9] + in[17] - in[5]);

    		t3 = in[1] + t1;
    		t4 = in[1] - t1 - t1;
    		t5 = t4 - t2;

    		t0 = mpeg2_cos9[0] * (in[5] + in[9]);
    		t1 = mpeg2_cos9[1] * (in[9] - in[17]);

    		tmp[13] = (t4 + t2 + t2) * mpeg2_tfcos36[17-13];
    		t2 = mpeg2_cos9[2] * (in[5] + in[17]);

    		t6 = t3 - t0 - t2;
    		t0 += t3 + t1;
    		t3 += t2 - t1;

    		t2 = mpeg2_cos18[0] * (in[3]  + in[11]);
    		t4 = mpeg2_cos18[1] * (in[11] - in[15]);
    		t7 = mpeg2_COS6_1 * in[7];

    		t1 = t2 + t4 + t7;
    		tmp[17] = (t0 + t1) * mpeg2_tfcos36[17-17];
    		tmp[9]  = (t0 - t1) * mpeg2_tfcos36[17-9];
    		t1 = mpeg2_cos18[2] * (in[3] + in[15]);
    		t2 += t1 - t7;

    		tmp[14] = (t3 + t2) * mpeg2_tfcos36[17-14];
    		t0 = mpeg2_COS6_1 * (in[11] + in[15] - in[3]);
    		tmp[12] = (t3 - t2) * mpeg2_tfcos36[17-12];

    		t4 -= t1 + t7;

    		tmp[16] = (t5 - t0) * mpeg2_tfcos36[17-16];
    		tmp[10] = (t5 + t0) * mpeg2_tfcos36[17-10];
    		tmp[15] = (t6 + t4) * mpeg2_tfcos36[17-15];
    		tmp[11] = (t6 - t4) * mpeg2_tfcos36[17-11];
	    }
...
#define DCT12_PART2 \
	in0 += in4 * mpeg2_COS6_2; \
                    	 \
	in4 = in0 + in2;     \
	in0 -= in2;          \
                    	 \
	in1 += in5 * mpeg2_COS6_2; \
                    	 \
	in5 = (in1 + in3) * mpeg2_tfcos12[0]; \
	in1 = (in1 - in3) * mpeg2_tfcos12[2]; \
                    	\
	in3 = in4 + in5;    \
	in4 -= in5;         \
                    	\
...

	{
...

    	{
    		...
    		{
        		float tmp2 = (in1 - in5) * mpeg2_tfcos12[1];
        		tmp0 = tmp1 + tmp2;
        		tmp1 -= tmp2;
    		}
...
static float mpeg2_xcos1[AC3_N / 4];
static float mpeg2_xsin1[AC3_N / 4];
static float mpeg2_xcos2[AC3_N / 8];
static float mpeg2_xsin2[AC3_N / 8];


/* 128 point bit-reverse LUT */
static unsigned char mpeg2_bit_reverse_512[] = 
{
...
static unsigned char mpeg2_bit_reverse_256[] = 
{
...
mpeg2_complex_t cmplx_mult(mpeg2_complex_t a, mpeg2_complex_t b)
{
	mpeg2_complex_t ret;
...
{
	int i, k;
	mpeg2_complex_t angle_step;
	mpeg2_complex_t current_angle;

/* Twiddle factors to turn IFFT into IMDCT */
	for(i = 0; i < AC3_N / 4; i++)
	{
		mpeg2_xcos1[i] = -cos(2.0f * M_PI * (8 * i + 1 ) / ( 8 * AC3_N)); 
		mpeg2_xsin1[i] = -sin(2.0f * M_PI * (8 * i + 1 ) / ( 8 * AC3_N));
	}
	
/* More twiddle factors to turn IFFT into IMDCT */
	for(i = 0; i < AC3_N / 8; i++)
	{
		mpeg2_xcos2[i] = -cos(2.0f * M_PI * (8 * i + 1 ) / ( 4 * AC3_N)); 
		mpeg2_xsin2[i] = -sin(2.0f * M_PI * (8 * i + 1 ) / ( 4 * AC3_N));
	}
...
inline void swap_cmplx(mpeg2_complex_t *a, mpeg2_complex_t *b)
{
	mpeg2_complex_t tmp;
...
void mpeg2audio_ac3_imdct_do_512(mpeg2audio_t *audio, 
		float data[], 
		float *y, 
		int step, 
		float *delay)
{
	...
	float *delay_ptr;
	float *window_ptr;
	mpeg2_complex_t *buf = audio->ac3_imdct_buf;

/* Pre IFFT complex multiply plus IFFT cmplx conjugate  */
	for(i = 0; i < AC3_N / 4; i++)
	{
		buf[i].real =   (data[AC3_N / 2 - 2 * i - 1] * mpeg2_xcos1[i]) - (data[2 * i] * mpeg2_xsin1[i]);
	    buf[i].imag = -((data[2 * i] * mpeg2_xcos1[i]) + (data[AC3_N / 2 - 2 * i - 1] * mpeg2_xsin1[i]));
	}

/* Bit reversed shuffling */
	for(i = 0; i < AC3_N / 4; i++)
	{ 
		k = mpeg2_bit_reverse_512[i];
		if(k < i)
...
/* Post IFFT complex multiply  plus IFFT complex conjugate*/
	for(i = 0; i < AC3_N / 4; i++)
	{
		tmp_a_r =  buf[i].real;
		tmp_a_i = -buf[i].imag;
		buf[i].real = (tmp_a_r * mpeg2_xcos1[i])  -  (tmp_a_i  * mpeg2_xsin1[i]);
		buf[i].imag = (tmp_a_r * mpeg2_xsin1[i])  +  (tmp_a_i  * mpeg2_xcos1[i]);
	}

	y_ptr = y;
	delay_ptr = delay;
	window_ptr = mpeg2_window;
...
void mpeg2audio_ac3_imdct_do_256(mpeg2audio_t *audio, 
	float data[], 
	float *y, 
	int step, 
	float *delay)
...
/* Pre IFFT complex multiply plus IFFT cmplx conjugate */
	for(k = 0; k < AC3_N / 8; k++)
	{
		...
		buf_1[k].real =    data[p] * mpeg2_xcos2[k] - data[q] * mpeg2_xsin2[k];
	    buf_1[k].imag = - (data[q] * mpeg2_xcos2[k] + data[p] * mpeg2_xsin2[k]); 
		buf_2[k].real =    data[p + 1] * mpeg2_xcos2[k] - data[q + 1] * mpeg2_xsin2[k];
	    buf_2[k].imag = - (data[q + 1] * mpeg2_xcos2[k] + data[p + 1] * mpeg2_xsin2[k]); 
	}

/* IFFT Bit reversed shuffling */
	for(i = 0; i < AC3_N / 8; i++)
	{
		k = mpeg2_bit_reverse_256[i];
		if(k < i)
		{
...
/* Post IFFT complex multiply */
	for(i = 0; i < AC3_N / 8; i++)
	{
		...
		buf_1[i].real = (tmp_a_r * mpeg2_xcos2[i])  -  (tmp_a_i  * mpeg2_xsin2[i]);
		buf_1[i].imag = (tmp_a_r * mpeg2_xsin2[i])  +  (tmp_a_i  * mpeg2_xcos2[i]);
		tmp_a_r =  buf_2[i].real;
		tmp_a_i = -buf_2[i].imag;
		buf_2[i].real = (tmp_a_r * mpeg2_xcos2[i])  -  (tmp_a_i  * mpeg2_xsin2[i]);
	    buf_2[i].imag = (tmp_a_r * mpeg2_xsin2[i])  +  (tmp_a_i  * mpeg2_xcos2[i]);
	}
...
int mpeg2audio_ac3_imdct(mpeg2audio_t *audio, 
		mpeg2_ac3bsi_t *bsi,
		...
		mpeg2ac3_stream_samples_t samples)
{
	...
	{
		if(audblk->blksw[i])
			mpeg2audio_ac3_imdct_do_256(audio, 
				samples[i], 
				audio->pcm_sample + audio->pcm_point + i, 
				bsi->nfchans, 
				audio->ac3_delay[i]);
		else
			mpeg2audio_ac3_imdct_do_512(audio, 
				samples[i], 
				audio->pcm_sample + audio->pcm_point + i, 
				bsi->nfchans, 
				audio->ac3_delay[i]);
