...
static inline int mpeg2audio_ac3_exp_unpack_ch(unsigned int type,
		unsigned int expstr,
		unsigned int ngrps,
...
/* Loop through the groups and fill the dest array appropriately */
	for(unsigned int i = 0; i < ngrps; i++)
	{
		if(exps[i] > 124)
		{
			_tprintf(TEXT("mpeg2audio_ac3_exp_unpack_ch: Invalid exponent %d\n"), exps[i]);
			return 1;
		}
		...
		mpeg2_ac3audblk_t *audblk)
{
	...
	for(i = 0; i < bsi->nfchans; i++)
		result |= mpeg2audio_ac3_exp_unpack_ch(UNPACK_FBW, 
			audblk->chexpstr[i], 
			audblk->nchgrps[i], 
			audblk->exps[i][0], 
			&audblk->exps[i][1], 
	...
	if(audblk->cplinu && !result)
		result |= mpeg2audio_ac3_exp_unpack_ch(UNPACK_CPL, 
			audblk->cplexpstr, 
			audblk->ncplgrps, 
			audblk->cplabsexp << 1,	
			audblk->cplexps, 
	...
	if(bsi->lfeon && !result)
		result |= mpeg2audio_ac3_exp_unpack_ch(UNPACK_LFE, 
			audblk->lfeexpstr, 
			2, 
			audblk->lfeexps[0], 
			&audblk->lfeexps[1], 
