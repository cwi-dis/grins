...
int mpeg2audio_decode_header(mpeg2audio_t *audio)
{
	...
	audio->bitrate = 1000 * mpeg2_tabsel_123[audio->lsf][audio->layer - 1][audio->bitrate_index];

    switch(audio->layer) 
	{
      	case 1:
        	audio->framesize  = (long)mpeg2_tabsel_123[audio->lsf][0][audio->bitrate_index] * 12000;
        	audio->framesize /= mpeg2_freqs[audio->sampling_frequency_code];
        	audio->framesize  = ((audio->framesize + audio->padding) << 2) - 4;
        	break;
      	case 2:
        	audio->framesize = (long)mpeg2_tabsel_123[audio->lsf][1][audio->bitrate_index] * 144000;
        	audio->framesize /= mpeg2_freqs[audio->sampling_frequency_code];
      	...
      	case 3:
        	...
        	if(audio->error_protection)
        	  	audio->ssize += 2;
        	audio->framesize  = (long)mpeg2_tabsel_123[audio->lsf][2][audio->bitrate_index] * 144000;
        	audio->framesize /= mpeg2_freqs[audio->sampling_frequency_code] << (audio->lsf);
...
int mpeg2audio_read_frame_body(mpeg2audio_t *audio)
{
	...
	{
		audio->bsbuf[i] = mpeg2bits_getbits(audio->astream, 8);
	}
	return 0;
}
...
int mpeg2audio_prev_header(mpeg2audio_t *audio)
{
	...
	{
		mpeg2bits_getbits_reverse(audio->astream, 8);
	}
/* Get reading in the forward direction again. */
	result |= mpeg2bits_refill(audio->astream);
	return result;
}
...
int mpeg2audio_read_header(mpeg2audio_t *audio)
{
	...
	{
		case AUDIO_AC3:
			result = mpeg2audio_read_ac3_header(audio);
			break;
...
/* Layer 1 not supported */
			if(audio->layer == 1)
			{
				_tprintf(TEXT("mpeg2audio_new: layer 1 not supported\n"));
				result = 1;
			}
			audio->newhead = mpeg2bits_showbits(audio->astream, 32);
			if(!mpeg2bits_eof(audio->astream) &&
				(mpeg2audio_head_check(audio->newhead) || mpeg2audio_decode_header(audio)))
			{
				do
				{
					try_var++;
					mpeg2bits_getbyte_noptr(audio->astream);
					audio->newhead = mpeg2bits_showbits(audio->astream, 32);
				}while(!mpeg2bits_eof(audio->astream) && 
					try_var < 65536 && 
					(mpeg2audio_head_check(audio->newhead) || mpeg2audio_decode_header(audio)));
    		}

/* Skip the 4 bytes containing the header */
			mpeg2bits_getbits(audio->astream, 32);
			break;
		
		case AUDIO_PCM:
			mpeg2audio_read_pcm_header(audio);
			break;
	}
