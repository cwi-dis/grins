...
int mpeg2audio_synth_stereo(mpeg2audio_t *audio, float *bandPtr, int channel, float *out, int *pnt)
{
	...
	if(audio->bo & 0x1)
	{
    	b0 = buf[0];
    	bo1 = audio->bo;
    	mpeg2audio_dct64(buf[1] + ((audio->bo + 1) & 0xf), buf[0] + audio->bo, bandPtr);
	}
	else 
	{
    	b0 = buf[1];
    	bo1 = audio->bo + 1;
    	mpeg2audio_dct64(buf[0] + audio->bo, buf[1] + audio->bo + 1, bandPtr);
	}
...
/*printf("%f %f %f\n", buf[0][0], buf[1][0], bandPtr[0]); */
	...
	{
    	register int j;
    	float *window = mpeg2_decwin + 16 - bo1;
...
int mpeg2audio_synth_mono(mpeg2audio_t *audio, float *bandPtr, float *samples, int *pnt)
{
	...
	ret = mpeg2audio_synth_stereo(audio, bandPtr, 0, samples_tmp, &pnt1);
	samples += *pnt;
