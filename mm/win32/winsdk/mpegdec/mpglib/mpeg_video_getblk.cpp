...
void mpeg_video::decode_mpeg1_intra_block(int comp,int dc_dct_pred[])
{
  int val, i, j, sign;
  unsigned int code;
  DCTtab *tab;
  ...
  for (i=1; ; i++)
  {
    ...
    m_bitstream->flush_buffer(tab->len);
    ...
    {
      i+= m_bitstream->get_bits(6);

      val = m_bitstream->get_bits(8);
      if (val==0)
        val = m_bitstream->get_bits(8);
      else if (val==128)
        val = m_bitstream->get_bits(8) - 256;
      else if (val>128)
        val -= 256;
    ...
    j = scan[ZIG_ZAG][i];
    val = (val*m_pld->quantizer_scale*m_pld->intra_quantizer_matrix[j]) >> 3;
...
void mpeg_video::decode_mpeg1_non_intra_block(int comp)
{
  int val, i, j, sign;
  unsigned int code;
  DCTtab *tab;
  ...
  for (i=0; ; i++)
  {
    ...
    j = scan[ZIG_ZAG][i];
    val = (((val<<1)+1)*m_pld->quantizer_scale*m_pld->non_intra_quantizer_matrix[j]) >> 4;
...
void mpeg_video::decode_mpeg2_intra_block(int comp,int dc_dct_pred[])
{
  ...
  //ld1 = (m_pld->scalable_mode==SC_DP) ? &base : ld;
  ld1 = m_pld;
  bp = ld1->block[comp];
 ...
 //     ld = &base;
  ...
  for (i=1; ; i++)
  {
    ...
    if (tab->run==65) /* escape */
    {
      ...
      {
        putchar(' ');
        Print_Bits(m_bitstream->next_bits(6),6,6);
      }
#endif /* TRACE */

      i+= run = m_bitstream->get_bits(6);

#ifdef TRACE
      if (Trace_Flag)
      {
        putchar(' ');
        Print_Bits(m_bitstream->next_bits(12),12,12);
      }
#endif /* TRACE */

      val = m_bitstream->get_bits(12);
      if ((val&2047)==0)
      {
        if (!Quiet_Flag)
...
void mpeg_video::decode_mpeg2_non_intra_block(int comp)
{
